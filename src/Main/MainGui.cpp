...
int main( int argc, char ** argv )
{
	...
	if (argc >= 2)
	{
		...
		if (index != -1)
			workpath = fileName.substr(0, index);
	}*/
	...
	App::Application::Config()["LoggingFile"] = "1"; //ZD
    try {
        // Init phase ===========================================================
    ...
    try {
        ...
        if (App::Application::Config()["RunMode"] == "Gui" ||
            ...
            //Gui::Application::runApplication(workpath);
        else
            App::Application::runApplication();
    }
 ...
 //   Base::Console().Log("%s terminating...\n",App::Application::Config()["ExeName"].c_str());
	//Base::Console().Log("argc:%d\n", argc);
	//Base::Console().Log("argv:%s\n", argv[0]);
	//Base::Console().Log("argv:%s\n", argv[1]);
	...
	//_CrtDumpMemoryLeaks();
	//判断\Users\userName\AppData\Roaming\FreeCAD还存在吗，不存在清空注册表（位置信息）
	//std::string configPath = App::Application::Config()["UserAppData"] + "user.cfg";
	//FILE * pFile = std::fopen(configPath.c_str(), "r");
	if (App::Application::Config()["FlagReset"]=="true")
	{
		try{
			//清空注册表
			std::string batClearConfig = App::Application::Config()["BinPath"] + "Delete User Config.bat";
			std::string batClearRegister = App::Application::Config()["BinPath"] + "Delete Window Position.bat";

			WinExec(batClearConfig.c_str(), SW_HIDE);
			WinExec(batClearRegister.c_str(), SW_HIDE);
		}
		catch (...)
		{
