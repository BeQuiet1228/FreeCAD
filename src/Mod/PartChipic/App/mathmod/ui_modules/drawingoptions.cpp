...
// --------------------------
void DrawingOptions::CalculateProgress(int p)
{
    if(p == 100)
    {
        if(Parameters->autoFilePath!="")
        {
                while(MathmodRef->ui.glWidget->IsoObjetThread->IsoObjet->isRunning())
                {
                    Sleep(1);
                }
               ExportObj(Parameters->autoFilePath.replace(".json",".obj"));
...
void DrawingOptions::LoadNewFileModels(QString path, bool upd)
{
    QString mathmodcollection = path;
    ...
    //设置选择
    on_choice_activated(lst[1]);
}

void DrawingOptions::ExportObj(QString path) const
{
    //MathmodRef->ui.glWidget->SaveSceneAsObjPoly(path);
    int type =0;
    auto LocalScene = MathmodRef->ui.glWidget->LocalScene;
    uint TypeDrawin=10;
    uint TypeDrawinNormStep = 4;

    QFile data(path.mid(0,path.length() -1));
    if (data.open(QFile::ReadWrite | QFile::Truncate))
    {
        ...
        for (i = 0; i < LocalScene.PolyNumber ; i+=3)
        {
            ...
            (stream) <<"f "<<"  "   <<LocalScene.PolyIndices_localPt[i + 2] +1<<"  "\
                     <<LocalScene.PolyIndices_localPt[i + 1] +1<<"  "\
                     <<LocalScene.PolyIndices_localPt[i + 0] +1<<"\n";
        }
...
void DrawingOptions::LoadK3DSurfScript (QString filename, int type)
{
    ...
    if(type == 1)
    {
        if ( file.open(  QIODevice::ReadOnly | QIODevice::Text ) )
        {
            ...
            QFile f( filename.replace(".k3ds", ".json"));
            if(f.exists())
                f.remove();
