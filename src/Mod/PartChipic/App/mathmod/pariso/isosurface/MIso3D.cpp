...
//+++++++++++++++++++++++++++++++++++++++++
void MIso3D::WorkerThreadCopy(IsoWorkerThread *WorkerThreadsTmp)
{
   
}
//+++++++++++++++++++++++++++++++++++++++
void MIso3D::UpdateThredsNumber(uint NewThreadsNumber)
{
}

//+++++++++++++++++++++++++++++++++++++++
ErrorMessage  MIso3D::IsoMorph()
{
...
//+++++++++++++++++++++++++++++++++++++++
ErrorMessage MIso3D::ThreadParsersCopy()
{

    return(parse_expression2());
}

ErrorMessage  MIso3D::parse_expression2()
{
...
/// +++++++++++++++++++++++++++++++++++++++++
MIso3D::MIso3D( uint maxtri, uint maxpts, uint nbmaxgrid,
              uint NbCompo,
              uint NbVariabl,
              uint NbConstant,
              uint NbDefinedFunct,
...
//+++++++++++++++++++++++++++++++++++++++++
void MIso3D::ReinitVarTablesWhenMorphActiv(uint IsoIndex)
{
...
//+++++++++++++++++++++++++++++++++++++++++
void IsoWorkerThread::VoxelEvaluation()
{
    double* vals;
    double* Res;
    ...
    //****
    implicitFunctionParser[0].AllocateStackMemory(Stack_Factor);
    //qDebug()<<"Xgrid="<<Xgrid<<" limitY="<<limitY<<" limitZ="<<limitZ;
    /*for(uint i=iStart; i<iFinish; i+=nbX )
    {
        ...
        for(uint j=0; j<limitY; j+=nbY)
        {
            ...
            for(uint k=0; k<limitZ; k+=nbZ)
            {
                ...
                for(uint l=0; l<nbstack; l++)
                {
                    vals[l*34  ]= xLocal2[Iindice+uint(l*nbX/nbstack)];
                    vals[l*34+1]= yLocal2[Jindice+((uint(l/nbZ))%nbY)];
                    vals[l*34+2]= zLocal2[Kindice+(l%nbZ)];

                    for(uint p=0; p<Nb_newvariables; p++)
                    {
                        vals[l*34+4 + p*3] = vr2[(p*3  )*NbComponent*NbMaxGrid + Iindice +uint(l*nbX/nbstack)];
                        vals[l*34+5 + p*3] = vr2[(p*3+1)*NbComponent*NbMaxGrid + Jindice +((uint(l/nbZ))%nbY)];
                        vals[l*34+6 + p*3] = vr2[(p*3+2)*NbComponent*NbMaxGrid + Kindice +(l%nbZ)];
                    }
                }

                IJK = J+Kindice;
                double res = implicitFunctionParser[0].Eval2(vals, 34, Res, nbstack);
                if( abs(res - IF_FUNCT_ERROR) == 0.0)
                {
                    for(uint l=0; l<nbstack; l++)
                        Res[l] = implicitFunctionParser[0].Eval(&(vals[l*34]));
                }
...
////                            //2020-0107
//                           if((i==0&&ii==0)||(i==Xgrid-1&&ii==nbX-1)||(j==0&&jj==0)||(j==limitY-1&&jj==nbY-1)||(k==0&&kk==0)||(k==limitZ-1&&kk==nbZ-1))
//                             if(Res[p]<0.0)
//                             {

//                                 //qDebug()<<"i="<<i<<" ii="<<ii<<" j="<<j<<" jj="<<jj<<" k="<<k<<" kk="<<kk;
//                                  Res[p]=0.125;
//                                 //Res[p]=0.25;
//                             }
	...
	for (uint i = iStart; i<iFinish; i ++)
	{
		I = i*maxgrscalemaxgr;
		for (uint j = 0; j<limitY; j ++)
		{
			J = I + j*maximumgrid;
			for (uint k = 0; k<limitZ; k ++)
			{
				...
				vals[1] = yLocal2[j];
				vals[2] = zLocal2[k];
				Results[IJK] = implicitFunctionParser[0].Eval(&(vals[0]));
...
/*	//i=0
	...
	for (uint i = iFinish - 1; i == iFinish - 1; i++)
	{
		I = i*maxgrscalemaxgr;
		for (uint j = 0; j<limitY; j++)
		{
	...
	{
		I = Iindice*maxgrscalemaxgr;
		for (uint j = limitY - 1; j == limitY - 1; j++)
		{
			J = I + Jindice*maximumgrid;
			for (uint k = 0; k<limitZ; k++)
			{
	...
	for (uint i = iStart; i<iFinish; i++)
	{
		...
		{
			J = I + Jindice*maximumgrid;
			for (uint k = limitZ - 1; k == limitZ - 1; k++)
			{
				IJK = J + k;
				if (Results[IJK] < 0)
					Results[IJK] = 0;
...
///+++++++++++++++++++++++++++++++++++++++++
void MIso3D::ConstructIsoNormale()
{
    ...
    //2020
    //uint size=_msize(NormVertexTab)/sizeof(float);
...
ErrorMessage IsoMasterThread::ParseExpression(std::string VariableListe)
{
    ...
    for(uint IsoIndex=0; IsoIndex<Nb_implicitfunctions+1; IsoIndex++)
    {
		...
		*/
		xLocal2[IsoIndex*NbMaxGrid] = xInfParser[IsoIndex].Eval(vals);// xSupParser[IsoIndex].Eval(vals);
		yLocal2[IsoIndex*NbMaxGrid] = yInfParser[IsoIndex].Eval(vals);// ySupParser[IsoIndex].Eval(vals);
		zLocal2[IsoIndex*NbMaxGrid] = zInfParser[IsoIndex].Eval(vals);// zSupParser[IsoIndex].Eval(vals);

		x_Step[IsoIndex] = (xSupParser[IsoIndex].Eval(vals) - xLocal2[IsoIndex*NbMaxGrid]) / (limitX - 1);
		y_Step[IsoIndex] = (ySupParser[IsoIndex].Eval(vals) - yLocal2[IsoIndex*NbMaxGrid]) / (limitY - 1);
		z_Step[IsoIndex] = (zSupParser[IsoIndex].Eval(vals) - zLocal2[IsoIndex*NbMaxGrid]) / (limitZ - 1);
...
///+++++++++++++++++++++++++++++++++++++++++
void IsoWorkerThread::IsoCompute()
{
    VoxelEvaluation();
}

///+++++++++++++++++++++++++++++++++++++++++
void MIso3D::stopcalculations(bool calculation)
{
...
//+++++++++++++++++++++++++++++++++++++++++
void MIso3D::copycomponent(struct ComponentInfos* copy, struct ComponentInfos* origin)
{
...
)
{
    ...
    for(uint fctnb= 0; fctnb< masterthread->Nb_implicitfunctions+1; fctnb++)
    {
        ...
        uint result = PointEdgeComputation();
        if(result == 0)
        {
...
///+++++++++++++++++++++++++++++++++++++++++
uint MIso3D::CNDtoUse(uint index, struct ComponentInfos *components)
{
    for(uint fctnb= 0; fctnb < (masterthread->Nb_implicitfunctions+1); fctnb++)
...
///+++++++++++++++++++++++++++++++++++++++++
void MIso3D::CalculateColorsPoints(struct ComponentInfos *components)
{
    double tmp,
            *ValCol,
            val[4];
...
///+++++++++++++++++++++++++++++++++++++++++
uint MIso3D::CNDCalculation(uint & NbTriangleIsoSurfaceTmp, struct ComponentInfos *components)
{
    double vals[4];
    vals[3] = masterthread->stepMorph;
...
///++++++++++++++++++++ ConstructIsoSurface +++++++++++++++++++++++++++
uint MIso3D::SetMiniMmeshStruct()
{
...
///++++++++++++++++++++ ConstructIsoSurface +++++++++++++++++++++++++++
uint MIso3D::ConstructIsoSurface()
{
...
///+++++++++++++++++++++++++++++++++++++++++
uint MIso3D::PointEdgeComputation()
{
        ...
        for(j = j_Start; j < j_End; j++)
        {
            ...
            for(k = k_Start; k < k_End; k++)
            {
                ...
                if(IsoValue_1 * IsoValue_2 <= 0 && (rapport=IsoValue_2 - IsoValue_1) != 0.0 )
                {
					...
					//if (i + 1 == i_End)
					//	vals[0] = masterthread->xLocal2[i_End];
					//else
                    vals[0] = masterthread->xLocal2[i] + factor * masterthread->x_Step[0];
                    vals[1] = masterthread->yLocal2[j];
                    vals[2] = masterthread->zLocal2[k];
                    ///===========================================================///
                ...
                if(IsoValue_1 * IsoValue_2 <= 0 && (rapport=IsoValue_2 - IsoValue_1) != 0.0)
                {
                    ...
                    vals[0] = masterthread->xLocal2[i];
					//if (j + 1 == j_End)
					//	vals[1] = masterthread->yLocal2[j_End];
					//else
						vals[1] = masterthread->yLocal2[j] + factor * masterthread->y_Step[0];
                    vals[2] = masterthread->zLocal2[k];
                ...
                if(IsoValue_1 * IsoValue_2 <= 0 && (rapport=IsoValue_2 - IsoValue_1) != 0.0)					
                {
                    ...
                    vals[1] = masterthread->yLocal2[j];
					if (k + 1 == k_End)
						vals[2] = masterthread->zLocal2[k_End];
					else
						vals[2] = masterthread->zLocal2[k] + factor * masterthread->z_Step[0];
    ...
    for(j=0; j < Ygrid; j++)
    {
        ...
        for(k=0; k < Zgrid; k++)
        {
            ...
            if(j == 0 || k == 0 || (IsoValue_1 * IsoValue_2 <= 0 && (rapport=IsoValue_2 - IsoValue_1) != 0.0))
            {
                // Edge Point computation and  save in IsoPointMap
                factor = (IsoValue - IsoValue_1)/rapport;
                index  = TypeDrawin*NbPointIsoMap_local;
    ...
    for(i=0; i < limitX; i++)
        for(k=0; k < Zgrid; k++)
        {
            ...
            // Edge Point computation and  save in IsoPointMap
            if((IsoValue_1 * IsoValue_2 <= 0 && (rapport=IsoValue_2 - IsoValue_1) != 0.0))//i == 0 || k == 0 || 
            {
                factor = (IsoValue - IsoValue_1)/rapport;
                index  = TypeDrawin*NbPointIsoMap_local;
    ...
    for(i=0; i < limitX; i++)
        for(j=0; j < Ygrid; j++)
        {
            ...
            // Edge Point computation and  save in IsoPointMap
            if((IsoValue_1 * IsoValue_2 <= 0 && (rapport=IsoValue_2 - IsoValue_1) != 0.0))//i == 0 || j == 0 || 
            {
                factor = (IsoValue - IsoValue_1)/rapport;
                index  = TypeDrawin*NbPointIsoMap_local;
        ...
        for(j=0; j < Ygrid; j++)
        {
            ...
            if( i != (limitX -1) )
            {
                ...
                if(IsoValue_1 * IsoValue_2 <= 0 && (rapport=IsoValue_2 - IsoValue_1) != 0.0)
                {
                    ...
                    // Edge Point computation and  save in IsoPointMap
					factor = 0;// (IsoValue - IsoValue_1) / rapport;
                    index  = TypeDrawin*NbPointIsoMap_local;
...
///+++++++++++++++++++++++++++++++++++++++++++++++++++++///
void MIso3D::SignatureComputation()
{
