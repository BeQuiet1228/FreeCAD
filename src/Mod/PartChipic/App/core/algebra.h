...
#define vector4 vcg::Point4f
#define vector3 vcg::Point3f
#define normal3 vcg::Point3f

enum SYSTEM {CARTESIAN, CYLINDRICAL, POLAR};
...
vcg::Point3f operator*(const matrix4& a, const vcg::Point3f& v);
/// Matrix multiplication
vcg::Point3f operator*(const vcg::Point3f& v, const matrix4& a);

/////////////////////////////////////////////////////////////////////////////
// vcg::Point4f

/// Matrix multiplication
vcg::Point4f operator*(const matrix4& a, const vcg::Point4f& v);
/// Matrix multiplication
vcg::Point4f operator*(const vcg::Point4f& v, const matrix4& a);
...
{
	const vcg::Point3f a(m[0][0], m[0][1], m[0][2]);
	const vcg::Point3f b(m[1][0], m[1][1], m[1][2]);
	const vcg::Point3f c(m[2][0], m[2][1], m[2][2]);
...
//{
//	const vector4 temp((a * vcg::Point4f(v[0], v[1], v[2], 1)) - (a * vcg::Point4f(0, 0, 0, 1)));
...
//{
//	const vector4 temp((a * vcg::Point4f(v[0], v[1], v[2], 1)) - (a * vcg::Point4f(0, 0, 0, 1)));
//	return normal3(temp[0], temp[1], temp[2]);
//}
//
///// Linear transformation
...
public:
	...
	// Constructors
	quaternion();
	quaternion(const double u, const vcg::Point3f& t);
...
public:
	...
	// Constructors ...
	angle_axis();
	angle_axis(const double Angle, const vcg::Point3f& Axis);
...
public:
	...
	euler_angles();
	euler_angles(vcg::Point3f Angles, AngleOrder Order);
...
inline const matrix4 translate3(const vcg::Point3f& v)
{
	return matrix4(
		vector4(1, 0, 0, v[0]),
		vector4(0, 1, 0, v[1]),
...
/// Returns a three-dimensional translation matrix
inline const matrix4 translate3(const double X, const double Y, const double Z)
{
	return matrix4(
		vector4(1, 0, 0, X),
		vector4(0, 1, 0, Y),
...
inline const matrix4 rotate3(const double Angle, vcg::Point3f Axis)
{
	const double c = cos(Angle), s = sin(Angle), t = 1.0 - c;

	Axis = vcg::Normalize(Axis);
...
inline const matrix4 rotate3(const vcg::Point3f& EulerAngles)
{
	matrix4 matrix = identity3();
	matrix = matrix * rotate3(EulerAngles[1], vcg::Point3f(0, 1, 0));
	matrix = matrix * rotate3(EulerAngles[0], vcg::Point3f(1, 0, 0));
	matrix = matrix * rotate3(EulerAngles[2], vcg::Point3f(0, 0, 1));
...
inline const quaternion rotate3(const matrix4& m)
{
	...
	if(trace > 0.0)
	{
		...
		return quaternion(0.25 / s, s * vcg::Point3f(m[2][1] - m[1][2], m[0][2] - m[2][0], m[1][0] - m[0][1]));
	}
	...
	if(d0 >= d1 && d0 >= d2)
	{
		...
		return quaternion(m[1][2] - m[2][1], vcg::Point3f(0.5, m[0][1] - m[1][0], m[0][2] - m[2][0])) / s;
	}
	else if(d1 >= d0 && d1 >= d2)
	{
		double s = std::sqrt(1.0 + d1 - d0 - d2) * 2.0;
		return quaternion(m[0][2] - m[2][0], vcg::Point3f(m[0][1] - m[1][0], 0.5, m[1][2] - m[2][1])) / s;
	}
	else
	{
		double s = std::sqrt(1.0 + d2 - d0 - d1) * 2.0;
		return quaternion(m[0][1] - m[1][0], vcg::Point3f(m[0][2] - m[2][0], m[1][2] - m[2][1], 0.5)) / s;
	}
}

/// Returns a three-dimensional scaling matrix that scales uniformly along each dimension.
inline const matrix4 scale3(const double S)
{
	return matrix4(
		vector4(S, 0, 0, 0),
		vector4(0, S, 0, 0),
...
inline const matrix4 scale3(const double X, const double Y, const double Z)
{
	return matrix4(
		vector4(X, 0, 0, 0),
		vector4(0, Y, 0, 0),
...
inline const vcg::Point3f look_vector(const matrix4& Matrix)
{
    vcg::Point3f p = (Matrix * vcg::Point3f(0, 0, 1));
        return vcg::Normalize(p);
}

/// Extract an "up" vector from a view matrix
inline const vcg::Point3f up_vector(const matrix4& Matrix)
{
    vcg::Point3f p = Matrix * vcg::Point3f(0, 1, 0);
        return vcg::Normalize(p);
}

/// Extract a "right" vector from a view matrix
inline const vcg::Point3f right_vector(const matrix4& Matrix)
{vcg::Point3f p = Matrix * vcg::Point3f(1, 0, 0);
        return vcg::Normalize(p);
}

/// Extract position from a view matrix
inline const vcg::Point3f position(const matrix4& Matrix)
{
	return vcg::Point3f(Matrix[0][3], Matrix[1][3], Matrix[2][3]);
}

/// Construct a view matrix from look, up, right, and position data
inline const matrix4 view_matrix(const vcg::Point3f& Look, const vcg::Point3f& Up, const vcg::Point3f& Position)
{
        vcg::Point3f look = Look;
        look = vcg::Normalize(look);
        vcg::Point3f right = (Up ^ look);
        right = vcg::Normalize(right);
        vcg::Point3f up = (look ^ right);
        up = vcg::Normalize(up);
...
inline const vcg::Point3f extract_translation(const matrix4& m)
{
	return(vcg::Point3f(m[0][3], m[1][3], m[2][3]));
}
...
/////////////////////////////////////////////////////////////////////////////
// vcg::Point3f implementation

inline vcg::Point3f operator*(const matrix4& a, const vcg::Point3f& v)
{
	const vcg::Point4f result = a * vcg::Point4f(v[0], v[1], v[2], 1);
	return vcg::Point3f(result[0] / result[3], result[1] / result[3], result[2] / result[3]);
}

inline vcg::Point3f operator*(const vcg::Point3f& v, const matrix4& a)
{
	return transpose(a) * v;
}

/////////////////////////////////////////////////////////////////////////////
// vcg::Point4f implementation

inline vcg::Point4f operator*(const matrix4& a, const vcg::Point4f& v)
{
#define ROWCOL(i) a.v[i][0]*v[0] + a.v[i][1]*v[1] + a.v[i][2]*v[2] + a.v[i][3]*v[3]
	return vcg::Point4f(ROWCOL(0), ROWCOL(1), ROWCOL(2), ROWCOL(3));
#undef ROWCOL // (i)
}

inline vcg::Point4f operator*(const vcg::Point4f& v, const matrix4& a)
{
	return transpose(a) * v;
}
...
inline matrix4 operator*(const matrix4& a, const matrix4& b)
{
#define ROWCOL(i, j) a.v[i][0]*b.v[0][j] + a.v[i][1]*b.v[1][j] + a.v[i][2]*b.v[2][j] + a.v[i][3]*b.v[3][j]
	return matrix4(
...
{ w = 1.0; v = vcg::Point3f(0, 0, 0); }

inline quaternion::quaternion(const double u, const vcg::Point3f& t)
{ w = u; v = t; }
...
{
    w = cos(AngleAxis.angle * 0.5);
    vcg::Point3f axis = AngleAxis.axis;
    axis = vcg::Normalize(axis);
    v = sin(AngleAxis.angle * 0.5) * axis;
}
...
{ w = cos(AngleAxis.angle * 0.5);
    vcg::Point3f axis = AngleAxis.axis;
    axis = vcg::Normalize(axis);
    v = sin(AngleAxis.angle * 0.5) * axis; return *this; }
...
{ return std::sqrt(w*w + v.SquaredNorm()); }
...
{ const double t = 2*w; w = w*w - v.SquaredNorm(); v *= t; return *this; }

inline quaternion& quaternion::AxisRotation(const double phi)
{ v = vcg::Normalize(v); v *= sin(phi/2.0); w = cos(phi/2); return *this; }
...
{ angle = 0.0; axis = vcg::Point3f(0, 0, 1); }

inline angle_axis::angle_axis(const double Angle, const vcg::Point3f& Axis)
{ angle = Angle; axis = Axis;axis=vcg::Normalize(axis); }
...
{ angle = Angle; axis[0] = X; axis[1] = Y; axis[2] = Z; axis = vcg::Normalize(axis); }

inline angle_axis::angle_axis(const double AngleAxis[4])
{ angle = AngleAxis[0]; axis[0] = AngleAxis[1]; axis[1] = AngleAxis[2]; axis[2] = AngleAxis[3]; axis = vcg::Normalize(axis); }
...
inline euler_angles::euler_angles(vcg::Point3f Angles, AngleOrder Order)
{
	n[0] = Angles[0];
	n[1] = Angles[1];
	n[2] = Angles[2];
...
inline euler_angles::euler_angles(const matrix4& Matrix, AngleOrder Order)
{
	...
	if(repetition == euler_angles::Repeats)
	{
		float sy = sqrt(Matrix[i][j]*Matrix[i][j] + Matrix[i][k]*Matrix[i][k]);
		if(sy > 16*FLT_EPSILON)
		{
			n[0] = std::atan2(Matrix[i][j], Matrix[i][k]);
			n[1] = std::atan2(sy, Matrix[i][i]);
			n[2] = std::atan2(Matrix[j][i], -Matrix[k][i]);
		}
		else
		{
			n[0] = std::atan2(-Matrix[j][k], Matrix[j][j]);
			n[1] = std::atan2(sy, Matrix[i][i]);
			n[2] = 0;
		}
	}
	...
	{
		float cy = sqrt(Matrix[i][i]*Matrix[i][i] + Matrix[j][i]*Matrix[j][i]);
		if(cy > 16*FLT_EPSILON)
		{
                        n[0] = std::atan2(Matrix[k][j], Matrix[k][k]);
                        n[1] = std::atan2(-Matrix[k][i], cy);
                        n[2] = std::atan2(Matrix[j][i], Matrix[i][i]);
		}
		else
		{
                        n[0] = std::atan2(-Matrix[j][k], Matrix[j][j]);
