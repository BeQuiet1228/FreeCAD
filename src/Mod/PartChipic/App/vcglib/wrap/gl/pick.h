...
public:
  ...
  static int PickVert(int x, int y, MESH_TYPE &m, std::vector<VertexPointer> &result, std::vector<int> &resultIndex, int width=4, int height=4,bool sorted=true)
  {
	  result.clear();
	  if(width==0 ||height==0) return 0;
	  long hits;
	  ...
	  if(sorted)
		  std::sort(H.begin(),H.end());
	  result.resize(H.size());
	  resultIndex.resize(H.size());
	  for(long ii=0;ii<hits;ii++){
		  VertexIterator vi=m.vert.begin();
		  resultIndex[ii] = H[ii].second;
		  advance(vi ,H[ii].second);
		  result[ii]=&*vi;
	  }
 ...
 // Visibility is computed according to the current depth buffer. 
	...
	//test region
	static int PickVertVisible(int x, int y, MESH_TYPE &m, std::vector<VertexPointer> &resultZ, std::vector<int> &resultZIndex, int width=4, int height=4, bool sorted=true)
	{
		// First step 
		double mm[16];
		double mp[16];
		...
		PickVert(x,y,m,result,resultIndex,width,height,sorted);
		float LocalEpsilon = 0.001f;
		for(size_t i =0;i<result.size();++i)
		{	
			Point3f v = (*(result[i])).P();
			GLdouble tx,ty,tz;
			gluProject(v.X(),v.Y(),v.Z(), mm,mp,vp, &tx,&ty,&tz);
			if(tx >=0 && tx<screenW && ty >=0 && ty<screenH)
			{
				...
				{
					resultZ.push_back(result[i]);
				    resultZIndex.push_back(resultIndex[i]);
				}
			}
		}
