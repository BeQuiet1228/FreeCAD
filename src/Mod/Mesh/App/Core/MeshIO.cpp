...
bool MeshOutput::SaveOBJ (std::ostream &out) const
{
	...
	PyObject * pFunc = NULL;
	//调用python文件名，当前测试python的文件名是getm3dFileInfo.py
	pModule = PyImport_ImportModule("GetM3dFileInfo");
	//调用函数
	pFunc = PyObject_GetAttrString(pModule, "getM3dFileInfo");
	//给python传参
	//函数调用的参数传递均是以元组的形式打包的，1表示参数的个数
	PyObject *pArgs = PyTuple_New(1);
	//0表示序号，指第一个参数；s表示传入的参数类型是string类型
	PyTuple_SetItem(pArgs, 0, Py_BuildValue("s", "success"));
	//使用C++的python接口调用该函数
	PyObject *pReturn = PyEval_CallObject(pFunc, pArgs);
	//接受python返回的值
	char* nResult;
	//s表示转换成string类型
	//注意：PyArg_Parse的最后一个参数，必须加上&符号
	PyArg_Parse(pReturn, "s", &nResult);
	
	out << nResult << std::endl;	
