...
import Units
import FunctionInstance as Instance
from Modeling.Common.Tools import ObjectsTools
def createFunction():
    App.ActiveDocument.openTransaction("Function")
    obj = App.ActiveDocument.addObject("Part::FeaturePython",ObjectsTools.ObjectType.Vol_Function)
    Instance.Function(obj)
    Instance.ViewProviderFunction(obj.ViewObject)
    ObjectsTools.setRandColor(obj)
    # group=App.ActiveDocument.getObjectsByLabel("函数体")
    ...
    #     group=App.ActiveDocument.addObject("App::DocumentObjectGroup","Vol_Function")
    #     group.Label="函数体"
    #     group.addObject(obj)
