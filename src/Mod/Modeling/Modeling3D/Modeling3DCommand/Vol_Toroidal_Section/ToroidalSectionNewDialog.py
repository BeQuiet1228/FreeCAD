...
class showToroidalSectionDialog(showObjDialog):
    def __init__(self, obj, parent=None):
        #手动调用父类构造函数。
        showObjDialog.__init__(self,obj,parent)
        #设置ui文件
        self.ui = ToroidalSectionDialog.Ui_Dialog()
        ...
        if FreeCAD.ActiveDocument.CoordinateSystem=="Rectangular":
            ...
            try:
                if point_rt_after[1]==2:
                    self.obj.setExpression('MajorRadius',point_rt_after[0])
                else:
                    self.obj.setExpression('MajorRadius',None)
                    self.obj.MajorRadius=point_rt_after[0]
            except:
                self.error=self.error+'MajorRadius'+'  '+str(point_rt_before)+'\n'
            ...
            try:
                if point_rt2_after[1]==2:
                    self.obj.setExpression('MinorRadius',point_rt2_after[0])
                else:
                    self.obj.setExpression('MinorRadius',None)
                    self.obj.MinorRadius=point_rt2_after[0]
            except:
                self.error=self.error+'MinorRadius'+'  '+str(point_rt2_before)+'\n'
...
class reshowToroidalSectionDialog(showToroidalSectionDialog):
    def deleteobj(self):
        pass
        ...
        if FreeCAD.ActiveDocument.CoordinateSystem=="Rectangular":
            ...
            for i in range(len(list1)):
                if list1[i][0] == 'Point1.x':
                    ...
                    str1=list1[i][1].replace('Param.','')
                    list2[11]=str1
                elif list1[i][0] == 'MajorRadius':
                    str1=list1[i][1].replace('Param.','')
                    list2[12]=str1
                elif list1[i][0] == 'MinorRadius':
                    str1=list1[i][1].replace('Param.','')
                    list2[13]=str1
            ...
            for x in range(len(list2)):
                if len(list2[x]) == 0:
                    if x == 0:
                        ...
                        list2[x]=str(ValueUnitslength(float(self.obj.MajorRadius.Value),length))+length
                    elif x==13:
                        list2[x]=str(ValueUnitslength(float(self.obj.MinorRadius.Value),length))+length
