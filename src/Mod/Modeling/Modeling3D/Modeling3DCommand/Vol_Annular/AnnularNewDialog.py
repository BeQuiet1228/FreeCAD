...
class showAnnularDialog(showObjDialog):
    def __init__(self, obj, parent=None):
        #手动调用父类构造函数。
        showObjDialog.__init__(self,obj,parent)
        #设置ui文件
        self.ui = AnnularDialog.Ui_Dialog()
        ...
        if FreeCAD.ActiveDocument.CoordinateSystem=="Rectangular":
            ...
            try:
                if point_rb_after[1]==2:
                    self.obj.setExpression('RadiusInside',point_rb_after[0])
                else:
                    self.obj.setExpression('RadiusInside',None)
                    self.obj.RadiusInside=point_rb_after[0]
            except:
                self.error=self.error+'RadiusInside'+'  '+str(point_rb_before)+'\n'

            point_rt_before=self.ui.lineEdit_8.text()
            point_rt_after=InputTools.Stringfunctions(point_rt_before)
            try:
                if point_rt_after[1]==2:
                    self.obj.setExpression('RadiusOutside',point_rt_after[0])
                else:
                    self.obj.setExpression('RadiusOutside',None)
                    self.obj.RadiusOutside=point_rt_after[0]
            except:
                self.error=self.error+'RadiusOutside'+'  '+str(point_rt_before)+'\n'
            ...
            pass
class reshowAnnularDialog(showAnnularDialog):
    def deleteobj(self):
        pass
        ...
        if FreeCAD.ActiveDocument.CoordinateSystem=="Rectangular":
            ...
            for i in range(len(list1)):
                if list1[i][0] == 'Point_1.x':
                    ...
                    str1=list1[i][1].replace('Param.','')
                    list2[5]=str1
                elif list1[i][0] == 'RadiusInside':
                    str1=list1[i][1].replace('Param.','')
                    list2[6]=str1
                elif list1[i][0] == 'RadiusOutside':
                    str1=list1[i][1].replace('Param.','')
                    list2[7]=str1
            ...
            for x in range(len(list2)):
                if len(list2[x]) == 0:
                    if x == 0:
                        ...
                        list2[x]=str(ValueUnitslength(float(self.obj.RadiusInside.Value),length))+length
                    elif x==7:
                        # number = filter(str.isdigit,  str(self.obj.Radius))
                        list2[x]=str(ValueUnitslength(float(self.obj.RadiusOutside.Value),length))+length
