...
#     def __init__(self,threadId,name,oblist,i,cyl,progress_bar,l):
...
#         self.l=l
#     def run(self):
#         FreeCAD.Console.PrintMessage("run"+str(self.i)+"\n")
...
#         abase.Shape=self.oblist[self.i].Shape
#         abase.ViewObject.Proxy=0
#         abase.ViewObject.DiffuseColor = self.oblist[self.i].ViewObject.DiffuseColor
#         abase.ViewObject.Transparency = self.oblist[self.i].ViewObject.Transparency
#         doc.recompute()
#         atool = doc.addObject("Part::Feature")
#         atool.Shape = self.cyl
#         atool.ViewObject.ShapeColor = self.oblist[self.i].ViewObject.ShapeColor  # (0.25,0.57,0.35)
#         atool.ViewObject.Transparency = self.oblist[self.i].ViewObject.Transparency
#         doc.recompute()
#         acut.Base = abase
#         acut.Tool = atool
...
class CrossSection(PySide.QtGui.QWidget):
    ...
    def initUI(self, parent):
        ...
        # 添加保存切片图按钮 add by chenjian
        self.radioButton_3 = QtGui.QRadioButton(self)
        self.radioButton_3.setGeometry(QtCore.QRect(margin + 168, ypos, slw, 20))
        self.radioButton_3.setText(_fromUtf8("Save cut picture"))
        self.radioButton_3.setChecked(False)
        self.radioButton_3.clicked.connect(self.onRadioButton3)
        self.radioButton_3.setObjectName(_fromUtf8("radioButton_3"))
        self.radioButton_3.setToolTip(_translate(
            "MainWindow", "Save cross-section as cut picture", None))
        ...
        self.CB_00.setEnabled(False)
        ypos = ypos + 30
        # button
        self.computeBtn=QtGui.QPushButton(self)
        self.computeBtn.setText(_fromUtf8(u"计算截面"))
        self.computeBtn.setGeometry(QtCore.QRect(margin, ypos , slw, 20))
        self.computeBtn.clicked.connect(self.oncomputeBtn)
    ...
    def hideObjects(self):
        ...
        if App.ActiveDocument is None:
            ...
            self.oblist = []
            objectsAll= App.ActiveDocument.Objects
            ...
            # group=App.ActiveDocument.getObjectsByLabel("ClipCross")
            # App.Console.PrintMessage(str(group)+"\n")
            # if len(group):
            #     App.Console.PrintMessage("0\n")
            #     group[0].addObject(self.cs)
            # else:
            #     App.Console.PrintMessage("1\n")
            #     group=App.ActiveDocument.addObject("App::DocumentObjectGroup","ClipCross")
            #     group.Label="ClipCross"
            #     group.addObject(self.cs)
    ...
    def restoreObjects(self):
        ...
        try:
            if self.CB_00.isChecked() == False:
                App.ActiveDocument.removeObject(self.cs.Name)
                # App.ActiveDocument.removeObject(self.cs)
        except Exception:
            None
    ...
    def updateGui(self):
        ...
        if self.xdir == 0.0 and self.ydir == 0.0 and self.zdir == 0.0:
            ...
            if self.cross_section_type == 1:
                ...
                progress_bar.start("Do clipping command...",len(self.oblist))
