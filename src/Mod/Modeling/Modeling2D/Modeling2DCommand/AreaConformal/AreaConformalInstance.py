...
from Modeling.Modeling2D.Modeling2DCommand.AreaConformal import AreaConformalDlgMain


class AreaConformal2D(DraftTools.Rectangle):
    def createObject(self):
        ...
        obj = FreeCAD.ActiveDocument.addObject("Part::Part2DObjectPython", "ConformalArea")
        ObjectCreate(obj)
        ...
        processObject(obj, Tools2D.ObjectType.AreaConformal)
        Draft._ViewProviderRectangle(obj.ViewObject)

        Form = AreaConformalDlgMain.ShowDialog(obj, True)
        Form.exec_()
        self.finish(cont=True)
        FreeCAD.ActiveDocument.commitTransaction()

    def GetResources(self):
        IconPath = FreeCAD.ConfigGet("AppHomePath") + "Mod/Modeling/Modeling2D/modeling2DResources/投影面.svg"
...
FreeCADGui.addCommand('CreateAreaConformal_2D', AreaConformal2D())
