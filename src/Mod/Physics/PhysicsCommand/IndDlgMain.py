#-*- coding: utf-8 -*-
import Physics.PhysicsGui.IndDlg
...
def show(type,className,itemUserName):
    ...
    if type == "new":
        ObjectDict[className] = IndShow("new",className)
        ...
        if className in ObjectDict.keys():
            ...
            ObjectDict[className] = IndShow(itemUserName,className)
            ObjectDict[className].flagUpdateItemName = True
            ObjectDict[className].setNameUnable()
            ObjectDict[className].setModal(False)
            ObjectDict[className].show()
...
class IndShow(PhysicsDialog):
    ...
    def ComboBox_Shadow_clicked(self):
        ...
        if self.ui.ComboBox_Shadow.currentIndex() == 0:
            # self.ui.LineEdit_Name.setText("Inductor")
    ...
    def onConfirm(self,FreeCAD_Comment_Dict,className):
        self.close()
        #设置颜色
        SetColorTools.setColor(self.ui.ComboBox_Shadow.currentText(),physicsType="Ind")
    ...
    def loadData(self,DlgData): 
        try:
            ...
            # 线圈直径
            self.ui.LineEdit_diam.setText(DlgData.data['Diam'])
            # 自感系数
            self.ui.checkBox_induc.setChecked(DlgData.data['INDUC_Checked'])
            self.ui.LineEdit_induc.setText(DlgData.data['INDUC'])
    ...
    def keepData(self,DlgData):
        ...
        DlgData.addData("Dlg_Type","Ind_Type")
        ...
        # 线圈直径
        DlgData.addData("Diam",self.ui.LineEdit_diam.text())
        # 自感系数
        DlgData.addData("INDUC_Checked",self.ui.checkBox_induc.isChecked())
        DlgData.addData("INDUC",self.ui.LineEdit_induc.text())
        FreeCAD.Console.PrintMessage("\n加载完数据")
