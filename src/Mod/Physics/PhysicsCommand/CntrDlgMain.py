...
import Physics.PhysicsGui.CntrDlg
import Simulation
...
def show(type,className,itemUserName):
    if type == "new":
        ObjectDict[className] = CntrShow("new",className)
        ...
        if className in ObjectDict.keys():
            ...
            sayz("load")
            ObjectDict[className] = CntrShow(className,className)
            ObjectDict[className].flagUpdateItemName = True
            ObjectDict[className].setNameUnable()
            ObjectDict[className].setModal(False)
            ObjectDict[className].show()
            ObjectDict[className].exec_()    
class CntrShow(PhysicsDialog):
    def __init__(self,DialogID,className,parent=None):
        PhysicsDialog.__init__(self, parent)
        self.ui = Physics.PhysicsGui.CntrDlg.Ui_Dialog_CntrDlg()
    ...
    def onConfirm(self,FreeCAD_Comment_Dict,className):
        ...
        if flag == 0:              
            ...
            itemData, oldData = ObservePalMain.addItem(u"等值观测", name, className)
            flag = 1
        #防止修改名称使得json重复
        if self.flagUpdateItemName:
    ...
    def loadData(self,DlgData):   
        try:     
            ...
            # 观测场
            self.ui.ComboBox.setCurrentIndex(self.ui.ComboBox.findText(DlgData.data['Observation_field']))
            # 定时器
            self.ui.ComboBox_timer.setCurrentIndex(self.ui.ComboBox_timer.findText(DlgData.data['Timer']))
            # 等值线
            self.ui.checkBox.setChecked(DlgData.data['Contour_Checked'])
            # 法向选择
    ...
    def keepData(self,DlgData):
        ...
        if self.ui.ComboBox_Shadow.currentIndex() == 0: 
            ...
            ObjectsTools.findObjByLabelWithoutOrderAndInvisible(self.ui.ComboBox_Shadow.currentIndex())
            DlgData.addData("isAppointArea",True)   
        DlgData.addData("Dlg_Type","Cntr_Type")     
        ...
        # 观测场
        DlgData.addData("Observation_field",self.ui.ComboBox.currentText())        
        # 定时器
        ...
        # 等值线
        DlgData.addData("Contour_Checked",self.ui.checkBox.isChecked())
        ...
        # cntr = M3DFileUtil()
        # cntr.addOrUpdateContourCommands(
        # DlgData.id,
        # DlgData.getData("Observation_field"),
        # DlgData.getData("Timer2MX"),
        # isShade=DlgData.getData("Contour_Checked"),
