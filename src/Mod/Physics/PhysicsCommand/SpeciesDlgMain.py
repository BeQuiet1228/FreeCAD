#-*- coding: utf-8 -*-
import Physics.PhysicsGui.SpeciesDlg
...
def show(type,className,itemUserName):
    #具体内容需要再添加
    FreeCAD.Console.PrintMessage("\nClassNameshow:  "+str(className)+"\n") 
    FreeCAD.Console.PrintMessage("ClassNameshow:  "+str(className)+"\n") 

    global  ObjectDict  
    if type == "new":
        ObjectDict[className] = SpeciesShow("new",className)
    ...
    #同样是点击修改面板，为什么这里直接进入1但是其他有些面板同样的操作进入的却是2？如emh
        ...
        if className in ObjectDict.keys():
            FreeCAD.Console.PrintError('\n进入到 1111')
            FreeCAD.Console.PrintError(ObjectDict.keys())
        ...
        #2           
            ...
            ObjectDict[className] = SpeciesShow(itemUserName,className)
            ObjectDict[className].setNameUnable()
            ObjectDict[className].flagUpdateItemName = True
            ObjectDict[className].setModal(False)
            ObjectDict[className].show()
...
class SpeciesShow(PhysicsDialog):
    def __init__(self,DialogID,className,parent=None):
        ...
        FreeCAD.Console.PrintError('\nuserNameBefore:  ' + str(self.userNameBefore))
        #用于判断是否进行名称更新
    ...
    def onConfirm(self,FreeCAD_Comment_Dict,className):
        ...
        if flag == 0:              
            ...
            itemData, oldData = BoundPalMain.addItem(u"新粒子定义", name, className)
            flag = 1
        #防止修改名称使得json重复
        if self.flagUpdateItemName:
            if not name == self.userNameBefore:
                JSON_CADComment = json.loads(FreeCAD.ActiveDocument.Begin,object_pairs_hook=OrderedDict)
                FreeCAD.Console.PrintError('\n修改名字\n'+str(name)+"    "+ str(self.userNameBefore))
    ...
    def keepData(self,DlgData):
        ...
        DlgData.addData("Dlg_Type","Species_Type")
        DlgData.addData("powerUnit",self.ui.LineEdit_Unitl.text())
        DlgData.addData("quality",self.ui.LineEdit_Quality.text())
        DlgData.addData("massUnit",self.ui.comboBox.currentText())
        DlgData.addData("className",self.className)
    ...
    def loadData(self,DlgData):
        try:
            ...
            self.ui.LineEdit_Unitl.setText(DlgData.data["powerUnit"])
            self.ui.LineEdit_Quality.setText(DlgData.data["quality"])
            self.ui.comboBox.setCurrentIndex(self.ui.comboBox.fineText(DlgData.data["massUnit"]))
        except :
