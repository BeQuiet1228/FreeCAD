#-*- coding: utf-8 -*-
import Physics.PhysicsGui.EmbDlg
...
def show(type,className,itemUserName):
    global  ObjectDict  
    if type == "new":
        ObjectDict[className] = EmbShow("new",className)
        ...
        if className in ObjectDict.keys():
            ...
            Em_loadData(ObjectDict[className].ui,oldData,"EMB_TYPE")
            #设置标记的旧名字
            ...
            sayz("open")
            ObjectDict[className] = EmbShow(itemUserName,className)
            ObjectDict[className].flagUpdateItemName = True
            ObjectDict[className].setNameUnable()
            ObjectDict[className].setModal(False)
            ObjectDict[className].show()
...
class EmbShow(PhysicsDialog):
    def __init__(self,DialogID,className,parent=None):
        ...
        if DialogID != "new":
            ...
            Em_loadData(self.ui,oldData,"EMB_TYPE")
            flag = 1 
    ...
    def onConfirm(self,FreeCAD_Comment_Dict,className):
        ...
        isModify = Em_keepData(self.ui,newData,"EMB_TYPE")
