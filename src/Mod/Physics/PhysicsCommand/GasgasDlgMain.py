#-*- coding: utf-8 -*-
import Physics.PhysicsGui.GasgasDlg
...
def show(type,className,itemUserName):
    ...
    global  ObjectDict  
    if type == "new":
        ObjectDict[className] = GasgasShow("new",className)
    ...
    #同样是点击修改面板，为什么这里直接进入1但是其他有些面板同样的操作进入的却是2？如emh
        ...
        #2           
            ...
            ObjectDict[className] = GasgasShow(className,className)
            ObjectDict[className].flagUpdateItemName = True
            ObjectDict[className].setModal(False)
            ObjectDict[className].show()
            ObjectDict[className].exec_()  
    pass
class GasgasShow(PhysicsDialog):
    def __init__(self,DialogID,className,parent=None):
        PhysicsDialog.__init__(self, parent)
        self.ui = Physics.PhysicsGui.GasgasDlg.Ui_Dialog()
    ...
    def keepData(self,DlgData):
        ...
        DlgData.addData("Dlg_Type","Gasgas_Type")
        DlgData.addData("Types",self.ui.comboBox_type.currentText())
        DlgData.addData("pressure",self.ui.lineEdit.text())
        DlgData.addData("temperature",self.ui.lineEdit_2.text())
    ...
    def loadData(self,DlgData):
        try:
            self.ui.comboBox_type.setCurrentIndex(self.ui.comboBox_type.findText(DlgData.data["Types"]))
            self.ui.lineEdit.setText(DlgData.data["pressure"])
            self.ui.lineEdit_2.setText(DlgData.data["temperature"])
        except :
