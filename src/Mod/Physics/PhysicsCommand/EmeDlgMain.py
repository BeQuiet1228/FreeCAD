...
import Physics.PhysicsGui.EmeDlg
...
def show(type,className,itemUserName):
    if type == "new":
        ObjectDict[className] = EmeShow("new",className)
        ...
        if className in ObjectDict.keys():
            ...
            Em_loadData(ObjectDict[className].ui, oldData, "EME_TYPE")
            # 设置标记的旧名字
            # FreeCAD.Console.PrintError('\neme对话框打开，加载json之后')
            ...
            sayz("open")
            ObjectDict[className] = EmeShow(itemUserName,className)
            ObjectDict[className].flagUpdateItemName = True
            ObjectDict[className].setNameUnable()
            ObjectDict[className].setModal(False)
            ObjectDict[className].show()
...
class EmeShow(PhysicsDialog):
    ...
    def onConfirm(self,FreeCAD_Comment_Dict,className):
        ...
        isModify = Em_keepData(self.ui,newData,"EME_TYPE")
    ...
    def checkBox_TField_clicked(self):
        self.ui.LineEdit_TField.setEnabled(self.ui.checkBox_TField.isChecked())
    def checkBox_RField_clicked(self):
        self.ui.LineEdit_RField.setEnabled(self.ui.checkBox_RField.isChecked())
    def checkBox_Charg_clicked(self):
        self.ui.LineEdit_Charg.setEnabled(self.ui.checkBox_Charg.isChecked())
    def checkBox_FRate_clicked(self):
        self.ui.LineEdit_FRate.setEnabled(self.ui.checkBox_FRate.isChecked())
