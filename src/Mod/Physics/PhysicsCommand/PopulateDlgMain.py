#-*- coding: utf-8 -*-
import Physics.PhysicsGui.PopulateDlg
...
def show(type,className,itemUserName):
    ...
    global  ObjectDict  
    if type == "new":
        ObjectDict[className] = PopulateShow("new",className)
    ...
    #同样是点击修改面板，为什么这里直接进入1但是其他有些面板同样的操作进入的却是2？如emh
        ...
        #2           
            ...
            ObjectDict[className] = PopulateShow(className,className)
            ObjectDict[className].flagUpdateItemName = True
            ObjectDict[className].setModal(False)
            ObjectDict[className].show()
            ObjectDict[className].exec_()  
    pass
class PopulateShow(PhysicsDialog):
    def __init__(self,DialogID,className,parent=None):
        PhysicsDialog.__init__(self, parent)
        self.ui = Physics.PhysicsGui.PopulateDlg.Ui_Dialog()
    ...
    def keepData(self, DlgData):
        ...
        DlgData.addData("Dlg_Type", "Populate_Type")
        # if self.ui.comboBox_2.currentText() == "离子":
        #     DlgData.addData("ParticalType", "ION")
        # else:
        #     DlgData.addData("ParticalType", "ELECTRON")
        DlgData.addData("ParticalType",self.ui.comboBox_2.currentText())
        DlgData.addData("Orthogonal_projection_surface", self.ui.comboBox.currentText())
        DlgData.addData("X1", self.ui.lineEdit_X1.text())
        DlgData.addData("Y1", self.ui.lineEdit_Y1.text())
        DlgData.addData("Z1", self.ui.lineEdit_Z1.text())
        DlgData.addData("X2", self.ui.lineEdit_X2.text())
        DlgData.addData("Y2", self.ui.lineEdit_Y2.text())
        DlgData.addData("Z2", self.ui.lineEdit_Z2.text())
        DlgData.addData("density", self.ui.lineEdit_density.text())
        DlgData.addData("temp", self.ui.lineEdit_temp.text())
    ...
    def loadData(self, DlgData):
        try:
            # self.ui.lineEdit_kind.setText(DlgData.data['ParticalType'])
            # if DlgData.data["ParticalType"] == "ELECTRON":
            #     self.ui.comboBox_2.setCurrentIndex(0)
            # else:
            #     self.ui.comboBox_2.setCurrentIndex(1)
            # 设置标记的旧名字
            self.userNameBefore = DlgData.data['name']
            self.ui.comboBox_2.setCurrentIndex(self.ui.comboBox_2.findText(DlgData.data["ParticalType"]))
            self.ui.comboBox.setCurrentIndex(self.ui.comboBox.findText(DlgData.data['Orthogonal_projection_surface']))
            self.ui.lineEdit_X1.setText(DlgData.data["X1"])
            self.ui.lineEdit_Y1.setText(DlgData.data["Y1"])
            self.ui.lineEdit_Z1.setText(DlgData.data["Z1"])
            self.ui.lineEdit_X2.setText(DlgData.data["X2"])
            self.ui.lineEdit_Y2.setText(DlgData.data["Y2"])
            self.ui.lineEdit_Z2.setText(DlgData.data["Z2"])
            self.ui.lineEdit_density.setText(DlgData.data["density"])
            self.ui.lineEdit_temp.setText(DlgData.data["temp"])
        except:
    ...
    def refreshCombox(self):  
        ...
        for i in range(self.ui.comboBox.count()):
            ComboBox_Shadow_list.append(self.ui.comboBox.itemText(i))
    ...
    def refreshComboxType(self):
        ...
        self.ui.comboBox_2.addItem("ELECTRON")
        self.ui.comboBox_2.addItem("PROTON")
        for i in new_list:
