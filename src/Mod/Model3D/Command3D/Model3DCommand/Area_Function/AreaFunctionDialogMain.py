...
class ShowDialog(BaseDialogMain.BaseModelDialog):
    ...
    def getInfoFromObj(self):
        ...
        self.pointWidget.ui.lineEdit_expression.setText(str(self.obj.Expression).replace(' ', ''))
        self.pointWidget.ui.spinBox_precision.setValue(self.obj.Precision)
    ...
    def setInfoToObj(self):
        ...
        self.obj.Expression = self.pointWidget.ui.lineEdit_expression.text()
        self.obj.Precision = self.pointWidget.ui.spinBox_precision.value()
        self.obj.recompute()
    ...
    def slotOk(self):
        ...
        if judge:
            QtGui.QMessageBox.information(None, "", "无法有效绘制函数面，请检查坐标。")
        else:
            self.isKeepData = True
            self.close()

    def judgePoint(self):
        # 如果两个点X,Y,Z有一个相同，则不能构成
