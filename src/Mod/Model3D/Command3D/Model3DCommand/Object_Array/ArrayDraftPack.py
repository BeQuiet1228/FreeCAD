...
def dimSymbol(symbol=None, invert=False):
    ...
    if symbol == 0:
        ...
        sayz("Draft.dimsymbol: Not implemented")
        return coin.SoSphere()
    ...
    for obj in objectslist:
        if obj:
            if obj.isDerivedFrom("App::DocumentObjectGroup") or (
                    (getType(obj) in ["Space", "Site"]) and hasattr(obj, "Group")):
                if getType(obj) == "Site":
                    if obj.Shape:
                        newlist.append(obj)
...
def printShape(shape):
    ...
    sayz("solids: ", len(shape.Solids))
    sayz("faces: ", len(shape.Faces))
    sayz("wires: ", len(shape.Wires))
    sayz("edges: ", len(shape.Edges))
    sayz("verts: ", len(shape.Vertexes))
    if shape.Faces:
...
def compareObjects(obj1, obj2):
    ...
    if obj1.TypeId != obj2.TypeId:
        sayz(obj1.Name + " and " + obj2.Name + " are of different types")
        ...
        for p in obj1.PropertiesList:
            if p in obj2.PropertiesList:
                if p in ["Shape", "Label"]:
                    ...
                    sayz("Objects have different placements. Distance between the 2: " + delta + " units")
                else:
                    if getattr(obj1, p) != getattr(obj2, p):
                        sayz("Property " + p + " has a different value")
            else:
                sayz("Property " + p + " doesn't exist in one of the objects")
...
def loadTexture(filename, size=None):
    ...
    if gui:
        ...
        try:
            ...
            sayz("Draft: unable to load texture")
            return None
        else:
            return img
    return None
...
def makeWire(pointslist, closed=False, placement=None, face=None, support=None):
    ...
    if not isinstance(pointslist, list):
        ...
        pointslist = nlist
    if len(pointslist) == 0:
        sayz("Invalid input points: ", pointslist)
    # print(pointslist)
    # print(closed)
...
def makeCopy(obj, force=None, reparent=False):
    ...
    if (getType(obj) == "Rectangle") or (force == "Rectangle"):
        ...
        sayz("Error: Object type cannot be copied")
        return None
...
def makeArray(baseObject, name, arrayType, *args):
    '''init makeArray(object,intervalX,intervalY,intervalZ,num,[name],[arrayType]) for linear array:
    Creates an array of the given object
    args: IntervalX, IntervalY, IntervalZ, Number, NumberX, NumberY, NumberZ, centerAxis, NumberPolar
    '''
    obj = FreeCAD.ActiveDocument.addObject("Part::FeaturePython", name)
    _Array(obj)
    obj.Base = baseObject

    if ObjectTools.hasThePropertyByObj(baseObject, "Order"):
        # 先记录下Order，删除baseObject的Order属性后再重新赋Order
        objOrder = baseObject.Order
        # obj.Label = "[" + str(obj.Order) + "]_" + name

    if ObjectTools.hasThePropertyByObj(baseObject, "Type"):
        obj.BaseType = baseObject.Type

    if hasattr(baseObject, "Attribute"):
        obj.Attribute = baseObject.Attribute

    obj.Base.removeProperty("Order")
    obj.Base.removeProperty("Type")

    obj.Order = objOrder

    import re
    # 去掉order
    baseObjectLabel = str(re.split(r"\[[0-9]\d*\]\_", baseObject.Label)[1])
    baseObject.Label = baseObjectLabel

    # 基本模型不可更换
    obj.setEditorMode("Base", 1)

    # 隐藏不需要的参数
    obj.setEditorMode("BaseType", 2)
    ...
    if arrayType == "linear":
        if len(args) != 4:
            sayzerr("the number of arg is wrong")
        else:
            # obj.IntervalX = args[0]
            # obj.IntervalY = args[1]
            # obj.IntervalZ = args[2]
            # obj.Number = args[3]
            ObjectTools.turnExpressionToProperty(obj, "IntervalX", args[0])
            ObjectTools.turnExpressionToProperty(obj, "IntervalY", args[1])
            ObjectTools.turnExpressionToProperty(obj, "IntervalZ", args[2])
            ObjectTools.turnExpressionToProperty(obj, "Number", args[3])
        ...
        if len(args) != 5:
            ...
            obj.OrthoFace = face
            if face == "XY":
                # obj.IntervalX = args[1]
                # obj.IntervalY = args[2]
                # obj.NumberX = args[3]
                # obj.NumberY = args[4]
                ObjectTools.turnExpressionToProperty(obj, "IntervalX", args[1])
                ObjectTools.turnExpressionToProperty(obj, "IntervalY", args[2])
                ObjectTools.turnExpressionToProperty(obj, "NumberX", args[3])
                ObjectTools.turnExpressionToProperty(obj, "NumberY", args[4])
            elif face == "XZ":
                # obj.IntervalX = args[1]
                # obj.IntervalZ = args[2]
                # obj.NumberX = args[3]
                ...
                ObjectTools.turnExpressionToProperty(obj, "IntervalZ", args[2])
                ObjectTools.turnExpressionToProperty(obj, "NumberX", args[3])
                ObjectTools.turnExpressionToProperty(obj, "NumberZ", args[4])
            elif face == "YZ":
                # obj.IntervalY = args[1]
                # obj.IntervalZ = args[2]
                # obj.NumberY = args[3]
                ...
                ObjectTools.turnExpressionToProperty(obj, "IntervalY", args[1])
                ObjectTools.turnExpressionToProperty(obj, "IntervalZ", args[2])
                ObjectTools.turnExpressionToProperty(obj, "NumberY", args[3])
                ObjectTools.turnExpressionToProperty(obj, "NumberZ", args[4])
            else:
                sayzerr("set OrthoFace error")
    elif arrayType == "polar":
        if len(args) != 2:
            sayzerr("the number of arg is wrong")
        else:
            centerAxis = args[0]
            obj.CenterAxis = centerAxis
            # obj.NumberPolar = args[1]
            ObjectTools.turnExpressionToProperty(obj, "NumberPolar", args[1])
            if centerAxis == "X":
    ...
    # test for m3dFile
    # import Modeling.Common.Tools.ModelingCommandForM3dFile as test
    # list = test.__getArrayObjectInfo(FreeCAD.ActiveDocument.CoordinateSystem, obj)
    # print list

    # test for remodeling
    # import Modeling.Common.Tools.ModelingByM3dFile as remodeling
    # obj = FreeCADGui.Selection.getSelection()[0]
    # remodeling.__createArrayObject(FreeCAD.ActiveDocument.CoordinateSystem,
    #                                FreeCAD.ActiveDocument.getObject(obj.Name),
    #                                [u'Array001', u'Cylinder001', 'linear', 'X', 'XY', 1, 1, 1, 2, 1, 10, 10,
    #                                 10])
    return obj


def makeArrayPolar(baseObject, name, arrayType, *args):
    obj = FreeCAD.activeDocument().addObject("Part::FeaturePython", name)
    _ArrayPolar(obj)
    obj.Base = baseObject
    ...
    # 基本模型不可更换
    obj.setEditorMode("Base", 1)
    obj.setEditorMode("CenterAxis", 1)
    obj.setEditorMode("OrthoFace", 1)
    ...
    if arrayType == "linear":
        if len(args) != 4:
            ...
            # obj.IntervalR = args[0]
            # obj.IntervalTheta = args[1]
            # obj.IntervalZ = args[2]
            # obj.Number = args[3]
            ObjectTools.turnExpressionToProperty(obj, "IntervalR", args[0])
            ObjectTools.turnExpressionToProperty(obj, "IntervalTheta", args[1])
            ObjectTools.turnExpressionToProperty(obj, "IntervalZ", args[2])
            ObjectTools.turnExpressionToProperty(obj, "Number", args[3])
    elif arrayType == "ortho":
        if len(args) != 4:
            ...
            # obj.IntervalR = args[0]
            # obj.IntervalZ = args[1]
            # obj.NumberR = args[2]
            # obj.NumberZ = args[3]
            ObjectTools.turnExpressionToProperty(obj, "IntervalR", args[0])
            ObjectTools.turnExpressionToProperty(obj, "IntervalZ", args[1])
            ObjectTools.turnExpressionToProperty(obj, "NumberR", args[2])
            ObjectTools.turnExpressionToProperty(obj, "NumberZ", args[3])

    elif arrayType == "polar":
        if len(args) != 1:
            sayzerr("the number of arg is wrong")
        else:
            # obj.NumberPolar = args[0]
            ObjectTools.turnExpressionToProperty(obj, "NumberPolar", args[0])
    if gui:
        _ViewProviderDraftArray(obj.ViewObject)
        # 隐藏基本模型
        baseObject.ViewObject.hide()
...
def makeArrayCylindrical(baseobject, arg1, arg2, arg3, arg4=None, name="Array"):
    obj = FreeCAD.activeDocument().addObject("Part::FeaturePython", name)
    _ArrayCylindrical(obj)
    obj.Base = baseobject
    obj.setEditorMode("Base", 1)
    obj.setEditorMode("Placement", 2)
    ...
    for obj in objectslist:
        ...
        if getType(obj) == "Point":
            ...
            if copy and obj.isDerivedFrom("Mesh::Feature"):
                sayz("Mesh copy not supported at the moment")  # TODO
            newobj = obj
...
def offset(obj, delta, copy=False, bind=False, sym=False, occ=False):
    ...
    if getType(obj) in ["Sketch", "Part"]:
        copy = True
        sayz("the offset tool is currently unable to offset a non-Draft object directly - Creating a copy")
    ...
    if occ:
        ...
        if getType(obj) == "Wire":
            if p:
                ...
                sayz("Draft.offset: Unable to duplicate this object")
            ...
            if not (newobj) and newwire:
                ...
                sayz("Draft.offset: Unable to create an offset")
        if newobj:
            formatObject(newobj, obj)
    else:
        newobj = None
...
def getDXF(obj, direction=None):
    ...
    if getType(obj) == "Dimension":
        ...
        try:
            ...
            sayz("Draft.getDXF: Unable to project ", obj.Label, " to ", direction)
        else:
            result += d

    else:
        sayz("Draft.getDXF: Unsupported object: ", obj.Label)
...
def getSVG(obj, scale=1, linewidth=0.35, fontsize=12, fillstyle="shape color", direction=None, linestyle=None,
    ...
    if hasattr(obj, "isDerivedFrom"):
        if obj.isDerivedFrom("App::DocumentObjectGroup"):
            ...
            for child in obj.Group:
                svg += getSVG(child, scale, linewidth, fontsize, fillstyle, direction, linestyle, color, linespacing,
                              techdraw)
            return svg
    ...
    def getDiscretized(edge):
        ...
        for i in range(d + 1):
            v = getProj(
                edge.valueAt(edge.FirstParameter + ((float(i) / d) * (edge.LastParameter - edge.FirstParameter))))
            if not edata:
                ...
                if iscircle or isellipse:
                    ...
                    if bspline.Degree > 3 or bspline.isRational():
                        try:
                            ...
                            sayz("Debug: unable to approximate bspline")
                    if bspline.Degree <= 3 and not bspline.isRational():
                        ...
                        sayz("Debug: one edge (hash ", e.hashCode(), \
    ...
    def getArrow(arrowtype, point, arrowsize, color, linewidth, angle=0):
        ...
        if obj.ViewObject.ArrowType == "Circle":
            ...
            sayz("getSVG: arrow type not implemented")
        return svg
    ...
    if not obj:
        ...
        if not obj.ViewObject:
            sayz("export of dimensions to SVG is only available in GUI mode")
        ...
        if not obj.ViewObject:
            ...
            if hasattr(obj.ViewObject.Proxy, "circle"):
                ...
                if hasattr(obj.ViewObject, "ArrowType"):
                    ...
                    u1 = getProj((prx.circle.valueAt(prx.circle.FirstParameter + arrowlength)).sub(
                        prx.circle.valueAt(prx.circle.FirstParameter)))
                    u2 = getProj((prx.circle.valueAt(prx.circle.LastParameter - arrowlength)).sub(
                ...
                # drawing text
                if obj.ViewObject.DisplayMode == "2D":
                    t = prx.circle.tangentAt(
                        prx.circle.FirstParameter + (prx.circle.LastParameter - prx.circle.FirstParameter) / 2.0)
                    t = getProj(t)
                    tangle = DraftVecUtils.angle(t)
                    if (tangle <= -math.pi / 2) or (tangle > math.pi / 2):
                        tangle = tangle + math.pi
                    tbase = getProj(prx.circle.valueAt(
                        prx.circle.FirstParameter + (prx.circle.LastParameter - prx.circle.FirstParameter) / 2.0))
        ...
        if not obj.ViewObject:
            sayz("export of texts to SVG is only available in GUI mode")
        else:
        ...
        if not obj.ViewObject:
            sayz("export of axes to SVG is only available in GUI mode")
        else:
            vobj = obj.ViewObject
            lorig = getLineStyle()
            fill = 'none'
                ...
                for p in pos:
                    ...
                    if hasattr(vobj.Proxy, "bubbletexts"):
                        if len(vobj.Proxy.bubbletexts) >= n:
                            ...
                            svg += 'transform="translate(' + str(center.x + rad / 4.0) + ',' + str(
                                center.y - rad / 3.0) + ') '
                            svg += 'scale(1,-1)"> '
        ...
        if not obj.ViewObject:
            sayz("export of spaces to SVG is only available in GUI mode")
        else:
        ...
        if FreeCAD.GuiUp:
            if hasattr(obj.ViewObject, "EndArrow") and hasattr(obj.ViewObject, "ArrowType") and (
                    len(obj.Shape.Vertexes) > 1):
...
def makeSketch(objectslist, autoconstraints=False, addTo=None,
    ...
    * radiusPrecision(-1): If <0, disable radius constraint. If =0, add indiviaul
        ...
        if tp in ["Circle", "Ellipse"]:
            ...
            if not DraftGeomUtils.isPlanar(shape):
                FreeCAD.Console.PrintError(
                    translate("draft", "The given object is not planar and cannot be converted into a sketch."))
                return None
            if rotation is None:
                # rotation = obj.Placement.Rotation
                ...
                if norm:
                    ...
                    FreeCAD.Console.PrintWarning(
                        translate("draft", "Unable to guess the normal direction of this object"))
    ...
    if rotation:
        ...
        sayz("-----error!!! rotation is still None...")
...
def heal(objlist=None, delete=True, reparent=True):
    ...
    if not objlist:
        ...
        sayz("Automatic mode: Healing whole document...")
        auto = True
    else:
        sayz("Manual mode: Force-healing selected objects...")
        ...
        if ftype in ["Part::FeaturePython", "App::FeaturePython", "Part::Part2DObjectPython",
            ...
            if (proxy == 1) or (dtype in ["Unknown", "Part"]) or (not auto):
                ...
                if ("Dimline" in props) and ("Start" in props):
                    ...
                    dellist.pop()
                    sayz("Object " + obj.Name + " is not healable")

    if not got:
        sayz("No object seems to need healing")
    else:
        sayz("Healed ", len(dellist), " objects")
...
def downgrade(objects, delete=False, force=None):
    ...
    if force:
        ...
        elif (len(objects) == 1) and (objects[0].isDerivedFrom("Part::Feature")) and (
                "Base" in objects[0].PropertiesList):
...
class _ViewProviderDraft:
    ...
    def __init__(self, vobj):
        ...
        vobj.addProperty("App::PropertyEnumeration", "Pattern", "Draft",
                         QT_TRANSLATE_NOOP("App::Property", "Defines a hatch pattern"))
        vobj.addProperty("App::PropertyFloat", "PatternSize", "Draft",
                         QT_TRANSLATE_NOOP("App::Property", "Sets the size of the pattern"))
...
class _Dimension(_DraftObject):
    ...
    def __init__(self, obj):
        ...
        obj.addProperty("App::PropertyVectorDistance", "Start", "Draft",
                        QT_TRANSLATE_NOOP("App::Property", "Startpoint of dimension"))
        obj.addProperty("App::PropertyVectorDistance", "End", "Draft",
                        QT_TRANSLATE_NOOP("App::Property", "Endpoint of dimension"))
        obj.addProperty("App::PropertyVector", "Normal", "Draft",
                        QT_TRANSLATE_NOOP("App::Property", "the normal direction of this dimension"))
        obj.addProperty("App::PropertyVector", "Direction", "Draft",
                        QT_TRANSLATE_NOOP("App::Property", "the normal direction of this dimension"))
        obj.addProperty("App::PropertyVectorDistance", "Dimline", "Draft",
                        QT_TRANSLATE_NOOP("App::Property", "Point through which the dimension line passes"))
        obj.addProperty("App::PropertyLink", "Support", "Draft",
                        QT_TRANSLATE_NOOP("App::Property", "The object measured by this dimension"))
        obj.addProperty("App::PropertyLinkSubList", "LinkedGeometry", "Draft",
                        QT_TRANSLATE_NOOP("App::Property", "The geometry this dimension is linked to"))
        obj.addProperty("App::PropertyLength", "Distance", "Draft",
                        QT_TRANSLATE_NOOP("App::Property", "The measurement of this dimension"))
        obj.addProperty("App::PropertyBool", "Diameter", "Draft", QT_TRANSLATE_NOOP("App::Property",
...
class _ViewProviderDimension(_ViewProviderDraft):
    ...
    def __init__(self, obj):
        ...
        obj.addProperty("App::PropertyInteger", "Decimals", "Draft",
                        QT_TRANSLATE_NOOP("App::Property", "The number of decimals to show"))
        obj.addProperty("App::PropertyLength", "ArrowSize", "Draft", QT_TRANSLATE_NOOP("App::Property", "Arrow size"))
        obj.addProperty("App::PropertyLength", "TextSpacing", "Draft",
                        QT_TRANSLATE_NOOP("App::Property", "The spacing between the text and the dimension line"))
        obj.addProperty("App::PropertyEnumeration", "ArrowType", "Draft",
        ...
        obj.addProperty("App::PropertyDistance", "ExtLines", "Draft",
                        QT_TRANSLATE_NOOP("App::Property", "Length of the extension lines"))
        obj.addProperty("App::PropertyBool", "FlipArrows", "Draft",
                        QT_TRANSLATE_NOOP("App::Property", "Rotate the dimension arrows 180 degrees"))
        obj.addProperty("App::PropertyBool", "FlipText", "Draft",
                        QT_TRANSLATE_NOOP("App::Property", "Rotate the dimension text 180 degrees"))
        obj.addProperty("App::PropertyBool", "ShowUnit", "Draft",
                        QT_TRANSLATE_NOOP("App::Property", "Show the unit suffix"))
        obj.addProperty("App::PropertyVectorDistance", "TextPosition", "Draft", QT_TRANSLATE_NOOP("App::Property",
                                                                                                  "The position of the text. Leave (0,0,0) for automatic position"))
        obj.addProperty("App::PropertyString", "Override", "Draft",
                        QT_TRANSLATE_NOOP("App::Property", "Text override. Use $dim to insert the dimension length"))
        obj.addProperty("App::PropertyString", "UnitOverride", "Draft", QT_TRANSLATE_NOOP("App::Property",
...
class _AngularDimension(_DraftObject):
    ...
    def __init__(self, obj):
        ...
        obj.addProperty("App::PropertyAngle", "FirstAngle", "Draft",
                        QT_TRANSLATE_NOOP("App::Property", "Start angle of the dimension"))
        obj.addProperty("App::PropertyAngle", "LastAngle", "Draft",
                        QT_TRANSLATE_NOOP("App::Property", "End angle of the dimension"))
        obj.addProperty("App::PropertyVectorDistance", "Dimline", "Draft",
                        QT_TRANSLATE_NOOP("App::Property", "Point through which the dimension line passes"))
        obj.addProperty("App::PropertyVectorDistance", "Center", "Draft",
                        QT_TRANSLATE_NOOP("App::Property", "The center point of this dimension"))
        obj.addProperty("App::PropertyVector", "Normal", "Draft",
                        QT_TRANSLATE_NOOP("App::Property", "The normal direction of this dimension"))
        obj.addProperty("App::PropertyLink", "Support", "Draft",
                        QT_TRANSLATE_NOOP("App::Property", "The object measured by this dimension"))
...
class _ViewProviderAngularDimension(_ViewProviderDraft):
    ...
    def updateData(self, obj, prop):
        if hasattr(self, "arc"):
            ...
            if m == "3D":
                ...
                for i in range(arcsegs + 1):
                    p = self.circle.valueAt(self.circle.FirstParameter + (
                                (self.circle.LastParameter - self.circle.FirstParameter) / arcsegs) * i)
                ...
                i1 = len(pts1)
                i2 = i1 + len(pts2)
                self.arc.coordIndex.setValues(0, len(pts1) + len(pts2) + 1,
                                              list(range(len(pts1))) + [-1] + list(range(i1, i2)))
                if (len(pts1) >= 3) and (len(pts2) >= 3):
                    self.circle1 = Part.Arc(Vector(pts1[0][0], pts1[0][1], pts1[0][2]),
                                            Vector(pts1[1][0], pts1[1][1], pts1[1][2]),
                                            Vector(pts1[-1][0], pts1[-1][1], pts1[-1][2])).toShape()
                    self.circle2 = Part.Arc(Vector(pts2[0][0], pts2[0][1], pts2[0][2]),
                                            Vector(pts2[1][0], pts2[1][1], pts2[1][2]),
                                            Vector(pts2[-1][0], pts2[-1][1], pts2[-1][2])).toShape()
            else:
                pts = []
                for i in range(arcsegs + 1):
                    p = self.circle.valueAt(self.circle.FirstParameter + (
            ...
            arrowlength = 4 * obj.ViewObject.ArrowSize.Value
            u1 = (self.circle.valueAt(self.circle.FirstParameter + arrowlength)).sub(
                self.circle.valueAt(self.circle.FirstParameter)).normalize()
            u2 = (self.circle.valueAt(self.circle.LastParameter)).sub(
                self.circle.valueAt(self.circle.LastParameter - arrowlength)).normalize()
            if hasattr(obj.ViewObject, "FlipArrows"):
                if obj.ViewObject.FlipArrows:
                    u1 = u1.negative()
                    u2 = u2.negative()
...
class _Rectangle(_DraftObject):
    ...
    def __init__(self, obj):
        ...
        obj.addProperty("App::PropertyDistance", "Length", "Draft",
                        QT_TRANSLATE_NOOP("App::Property", "Length of the rectangle"))
        obj.addProperty("App::PropertyDistance", "Height", "Draft",
                        QT_TRANSLATE_NOOP("App::Property", "Height of the rectangle"))
        obj.addProperty("App::PropertyLength", "FilletRadius", "Draft",
                        QT_TRANSLATE_NOOP("App::Property", "Radius to use to fillet the corners"))
        obj.addProperty("App::PropertyLength", "ChamferSize", "Draft",
                        QT_TRANSLATE_NOOP("App::Property", "Size of the chamfer to give to the corners"))
        obj.addProperty("App::PropertyBool", "MakeFace", "Draft", QT_TRANSLATE_NOOP("App::Property", "Create a face"))
        obj.addProperty("App::PropertyInteger", "Rows", "Draft",
                        QT_TRANSLATE_NOOP("App::Property", "Horizontal subdivisions of this rectangle"))
        obj.addProperty("App::PropertyInteger", "Columns", "Draft",
                        QT_TRANSLATE_NOOP("App::Property", "Vertical subdivisions of this rectangle"))
...
class _ViewProviderRectangle(_ViewProviderDraft):
    def __init__(self, vobj):
        _ViewProviderDraft.__init__(self, vobj)
        vobj.addProperty("App::PropertyFile", "TextureImage", "Draft",
                         QT_TRANSLATE_NOOP("App::Property", "Defines a texture image (overrides hatch patterns)"))
...
class _Circle(_DraftObject):
    ...
    def __init__(self, obj):
        ...
        obj.addProperty("App::PropertyAngle", "FirstAngle", "Draft",
                        QT_TRANSLATE_NOOP("App::Property", "Start angle of the arc"))
        obj.addProperty("App::PropertyAngle", "LastAngle", "Draft", QT_TRANSLATE_NOOP("App::Property",
                                                                                      "End angle of the arc (for a full circle, give it same value as First Angle)"))
        obj.addProperty("App::PropertyLength", "Radius", "Draft",
                        QT_TRANSLATE_NOOP("App::Property", "Radius of the circle"))
        obj.addProperty("App::PropertyBool", "MakeFace", "Draft", QT_TRANSLATE_NOOP("App::Property", "Create a face"))
        obj.MakeFace = getParam("fillmode", True)
    ...
    def execute(self, obj):
        import Part
        plm = obj.Placement
        shape = Part.makeCircle(obj.Radius.Value, Vector(0, 0, 0), Vector(0, 0, 1), obj.FirstAngle.Value,
                                obj.LastAngle.Value)
...
class _Ellipse(_DraftObject):
    ...
    def __init__(self, obj):
        ...
        obj.addProperty("App::PropertyAngle", "LastAngle", "Draft", QT_TRANSLATE_NOOP("App::Property",
                                                                                      "End angle of the arc (for a full circle, give it same value as First Angle)"))
        obj.addProperty("App::PropertyLength", "MinorRadius", "Draft",
                        QT_TRANSLATE_NOOP("App::Property", "The minor radius of the ellipse"))
        obj.addProperty("App::PropertyLength", "MajorRadius", "Draft",
                        QT_TRANSLATE_NOOP("App::Property", "The major radius of the ellipse"))
        obj.addProperty("App::PropertyBool", "MakeFace", "Draft", QT_TRANSLATE_NOOP("App::Property", "Create a face"))
        obj.MakeFace = getParam("fillmode", True)
...
class _Wire(_DraftObject):
    ...
    def __init__(self, obj):
        ...
        obj.addProperty("App::PropertyVectorList", "Points", "Draft",
                        QT_TRANSLATE_NOOP("App::Property", "The vertices of the wire"))
        obj.addProperty("App::PropertyBool", "Closed", "Draft",
                        QT_TRANSLATE_NOOP("App::Property", "If the wire is closed or not"))
        obj.addProperty("App::PropertyLink", "Base", "Draft",
                        QT_TRANSLATE_NOOP("App::Property", "The base object is the wire, it's formed from 2 objects"))
        obj.addProperty("App::PropertyLink", "Tool", "Draft",
                        QT_TRANSLATE_NOOP("App::Property", "The tool object is the wire, it's formed from 2 objects"))
        obj.addProperty("App::PropertyVectorDistance", "Start", "Draft",
                        QT_TRANSLATE_NOOP("App::Property", "The start point of this line"))
        obj.addProperty("App::PropertyVectorDistance", "End", "Draft",
                        QT_TRANSLATE_NOOP("App::Property", "The end point of this line"))
        obj.addProperty("App::PropertyLength", "Length", "Draft",
                        QT_TRANSLATE_NOOP("App::Property", "The length of this line"))
        obj.addProperty("App::PropertyLength", "FilletRadius", "Draft",
                        QT_TRANSLATE_NOOP("App::Property", "Radius to use to fillet the corners"))
        obj.addProperty("App::PropertyLength", "ChamferSize", "Draft",
                        QT_TRANSLATE_NOOP("App::Property", "Size of the chamfer to give to the corners"))
        obj.addProperty("App::PropertyBool", "MakeFace", "Draft",
                        QT_TRANSLATE_NOOP("App::Property", "Create a face if this object is closed"))
        obj.addProperty("App::PropertyInteger", "Subdivisions", "Draft",
                        QT_TRANSLATE_NOOP("App::Property", "The number of subdivisions of each edge"))
        obj.MakeFace = getParam("fillmode", True)
        obj.Closed = False
...
class _ViewProviderWire(_ViewProviderDraft):
    ...
    def __init__(self, obj):
        ...
        obj.addProperty("App::PropertyBool", "EndArrow", "Draft",
                        QT_TRANSLATE_NOOP("App::Property", "Displays a dim symbol at the end of the wire"))
        obj.addProperty("App::PropertyLength", "ArrowSize", "Draft", QT_TRANSLATE_NOOP("App::Property", "Arrow size"))
        obj.addProperty("App::PropertyEnumeration", "ArrowType", "Draft",
                        QT_TRANSLATE_NOOP("App::Property", "Arrow type"))
        obj.ArrowSize = getParam("arrowsize", 0.1)
...
class _Polygon(_DraftObject):
    ...
    def __init__(self, obj):
        ...
        obj.addProperty("App::PropertyInteger", "FacesNumber", "Draft",
                        QT_TRANSLATE_NOOP("App::Property", "Number of faces"))
        obj.addProperty("App::PropertyLength", "Radius", "Draft",
                        QT_TRANSLATE_NOOP("App::Property", "Radius of the control circle"))
        obj.addProperty("App::PropertyEnumeration", "DrawMode", "Draft",
                        QT_TRANSLATE_NOOP("App::Property", "How the polygon must be drawn from the control circle"))
        obj.addProperty("App::PropertyLength", "FilletRadius", "Draft",
                        QT_TRANSLATE_NOOP("App::Property", "Radius to use to fillet the corners"))
        obj.addProperty("App::PropertyLength", "ChamferSize", "Draft",
                        QT_TRANSLATE_NOOP("App::Property", "Size of the chamfer to give to the corners"))
...
class _DrawingView(_DraftObject):
    ...
    def __init__(self, obj):
        ...
        obj.addProperty("App::PropertyVector", "Direction", "Shape View",
                        QT_TRANSLATE_NOOP("App::Property", "Projection direction"))
        obj.addProperty("App::PropertyFloat", "LineWidth", "View Style",
                        QT_TRANSLATE_NOOP("App::Property", "The width of the lines inside this object"))
        obj.addProperty("App::PropertyLength", "FontSize", "View Style",
                        QT_TRANSLATE_NOOP("App::Property", "The size of the texts inside this object"))
        obj.addProperty("App::PropertyLength", "LineSpacing", "View Style",
                        QT_TRANSLATE_NOOP("App::Property", "The spacing between lines of text"))
        obj.addProperty("App::PropertyColor", "LineColor", "View Style",
                        QT_TRANSLATE_NOOP("App::Property", "The color of the projected objects"))
        obj.addProperty("App::PropertyLink", "Source", "Base", QT_TRANSLATE_NOOP("App::Property", "The linked object"))
        obj.addProperty("App::PropertyEnumeration", "FillStyle", "View Style",
                        QT_TRANSLATE_NOOP("App::Property", "Shape Fill Style"))
        obj.addProperty("App::PropertyEnumeration", "LineStyle", "View Style",
                        QT_TRANSLATE_NOOP("App::Property", "Line Style"))
        obj.addProperty("App::PropertyBool", "AlwaysOn", "View Style", QT_TRANSLATE_NOOP("App::Property",
                                                                                         "If checked, source objects are displayed regardless of being visible in the 3D model"))
                    ...
                    for o in objs:
                        ...
                        if hasattr(obj, "AlwaysOn"):
                            if obj.AlwaysOn:
                                v = True
                        if v:
                            svg += getSVG(o, obj.Scale, obj.LineWidth, obj.FontSize.Value, obj.FillStyle, obj.Direction,
                                          ls, lc, lp)
                else:
                    svg = getSVG(obj.Source, obj.Scale, obj.LineWidth, obj.FontSize.Value, obj.FillStyle, obj.Direction,
                                 ls, lc, lp)
...
class _BSpline(_DraftObject):
    ...
    def __init__(self, obj):
        ...
        obj.addProperty("App::PropertyVectorList", "Points", "Draft",
                        QT_TRANSLATE_NOOP("App::Property", "The points of the B-spline"))
        obj.addProperty("App::PropertyBool", "Closed", "Draft",
                        QT_TRANSLATE_NOOP("App::Property", "If the B-spline is closed or not"))
        obj.addProperty("App::PropertyBool", "MakeFace", "Draft",
                        QT_TRANSLATE_NOOP("App::Property", "Create a face if this spline is closed"))
        obj.MakeFace = getParam("fillmode", True)
        obj.Closed = False
        obj.Points = []
    ...
    def assureProperties(self, obj):  # for Compatibility with older versions
        if not hasattr(obj, "Parameterization"):
            obj.addProperty("App::PropertyFloat", "Parameterization", "Draft",
                            QT_TRANSLATE_NOOP("App::Property", "Parameterization factor"))
...
class _BezCurve(_DraftObject):
    ...
    def __init__(self, obj):
        ...
        obj.addProperty("App::PropertyVectorList", "Points", "Draft",
                        QT_TRANSLATE_NOOP("App::Property", "The points of the Bezier curve"))
        obj.addProperty("App::PropertyInteger", "Degree", "Draft",
                        QT_TRANSLATE_NOOP("App::Property", "The degree of the Bezier function"))
        obj.addProperty("App::PropertyIntegerList", "Continuity", "Draft",
                        QT_TRANSLATE_NOOP("App::Property", "Continuity"))
        obj.addProperty("App::PropertyBool", "Closed", "Draft",
                        QT_TRANSLATE_NOOP("App::Property", "If the Bezier curve should be closed or not"))
        obj.addProperty("App::PropertyBool", "MakeFace", "Draft",
                        QT_TRANSLATE_NOOP("App::Property", "Create a face if this curve is closed"))
        obj.MakeFace = getParam("fillmode", True)
        obj.Closed = False
        obj.Degree = 3
        obj.Continuity = []
...
class _Block(_DraftObject):
    ...
    def __init__(self, obj):
        ...
        obj.addProperty("App::PropertyLinkList", "Components", "Draft",
                        QT_TRANSLATE_NOOP("App::Property", "The components of this block"))
...
class _Shape2DView(_DraftObject):
    ...
    def __init__(self, obj):
        obj.addProperty("App::PropertyLink", "Base", "Draft",
                        QT_TRANSLATE_NOOP("App::Property", "The base object this 2D view must represent"))
        obj.addProperty("App::PropertyVector", "Projection", "Draft",
                        QT_TRANSLATE_NOOP("App::Property", "The projection vector of this object"))
        obj.addProperty("App::PropertyEnumeration", "ProjectionMode", "Draft",
                        QT_TRANSLATE_NOOP("App::Property", "The way the viewed object must be projected"))
        obj.addProperty("App::PropertyIntegerList", "FaceNumbers", "Draft", QT_TRANSLATE_NOOP("App::Property",
                                                                                              "The indices of the faces to be projected in Individual Faces mode"))
        obj.addProperty("App::PropertyBool", "HiddenLines", "Draft",
                        QT_TRANSLATE_NOOP("App::Property", "Show hidden lines"))
        obj.addProperty("App::PropertyBool", "Tessellation", "Draft",
                        QT_TRANSLATE_NOOP("App::Property", "Tessellate Ellipses and B-splines into line segments"))
        obj.addProperty("App::PropertyBool", "InPlace", "Draft", QT_TRANSLATE_NOOP("App::Property",
                                                                                   "For Cutlines and Cutfaces modes, this leaves the faces at the cut location"))
        obj.addProperty("App::PropertyFloat", "SegmentLength", "Draft", QT_TRANSLATE_NOOP("App::Property",
                                                                                          "Length of line segments if tessellating Ellipses or B-splines into line segments"))
        obj.addProperty("App::PropertyBool", "VisibleOnly", "Draft", QT_TRANSLATE_NOOP("App::Property",
                                                                                       "If this is True, this object will be recomputed only if it is visible"))
...
class _Array(_DraftObject):
    ...
    def __init__(self, obj):
        ...
        obj.addProperty("App::PropertyLink", "Base", "Object of an Array",
                        QT_TRANSLATE_NOOP("App::Property", "The base object that must be duplicated"))
        obj.addProperty("App::PropertyEnumeration", "ArrayType", "Object of an Array",
                        QT_TRANSLATE_NOOP("App::Property", "The type of array to create"))
        obj.addProperty("App::PropertyEnumeration", "CenterAxis", "Object of an Array",
                        QT_TRANSLATE_NOOP("App::Property", "The axis of array to rotate"))
        obj.addProperty("App::PropertyEnumeration", "OrthoFace", "Object of an Array",
                        QT_TRANSLATE_NOOP("App::Property", "The face of copies"))
        obj.addProperty("App::PropertyVector", "Axis", "Object of an Array",
                        QT_TRANSLATE_NOOP("App::Property", "The axis direction"))
        obj.addProperty("App::PropertyInteger", "NumberX", "Object of an Array",
                        QT_TRANSLATE_NOOP("App::Property", "Number of copies in X direction"))
        obj.addProperty("App::PropertyInteger", "NumberY", "Object of an Array",
                        QT_TRANSLATE_NOOP("App::Property", "Number of copies in Y direction"))
        obj.addProperty("App::PropertyInteger", "NumberZ", "Object of an Array",
                        QT_TRANSLATE_NOOP("App::Property", "Number of copies in Z direction"))
        obj.addProperty("App::PropertyInteger", "Number", "Object of an Array",
                        QT_TRANSLATE_NOOP("App::Property", "Number of copies in linear"))
        obj.addProperty("App::PropertyInteger", "NumberPolar", "Object of an Array",
                        QT_TRANSLATE_NOOP("App::Property", "Number of copies"))
        obj.addProperty("App::PropertyDistance", "IntervalX", "Object of an Array",
                        QT_TRANSLATE_NOOP("App::Property", "Distance and orientation of intervals in X direction"))
        obj.addProperty("App::PropertyDistance", "IntervalY", "Object of an Array",
                        QT_TRANSLATE_NOOP("App::Property", "Distance and orientation of intervals in Y direction"))
        obj.addProperty("App::PropertyDistance", "IntervalZ", "Object of an Array",
                        QT_TRANSLATE_NOOP("App::Property", "Distance and orientation of intervals in Z direction"))
        obj.addProperty("App::PropertyString", "Type", "", "Type of Object").Type = ObjectTools.ObjectType.Vol_Array
        obj.addProperty("App::PropertyString", "BaseType", "", "Type of Base Object").BaseType = "BaseType"
        ...
        obj.Attribute = [ObjectTools.Attribute.NotDefine, ObjectTools.Attribute.Conductor, ObjectTools.Attribute.Custom,
                         ObjectTools.Attribute.Vacuo]

        obj.ArrayType = ['linear', 'ortho', 'polar']
        obj.CenterAxis = ['X', 'Y', 'Z']
        obj.OrthoFace = ['XY', 'XZ', 'YZ']
        obj.NumberX = 1
        obj.NumberY = 1
        obj.NumberZ = 1
        obj.Number = 1
    ...
    def onChanged(self, fp, prop):
        ...
        if prop == "Label":
            ...
            if self.flagLabel:
                self.flagLabel = False
                # 这里加个判断是为了解决b=FreeCAD.ActiveDocument.copyObject(o)，copy时不能读到labelBefore
                if not ObjectsTools.hasThePropertyByObj(fp, "labelBefore"):
                    ObjectsTools.updateWhenLableChanged(fp, fp.Label, fp.Label)
                else:
                    ObjectsTools.updateWhenLableChanged(fp, self.labelBefore, fp.Label)
                pass
                self.flagLabel = True
        if prop == "Attribute":
            if fp.Base:
                fp.Base.Attribute = fp.Attribute

    def execute(self, obj):
        FreeCAD.Console.PrintMessage("Array obj:")
        import DraftGeomUtils
        if obj.Base:
            pl = obj.Placement
            if obj.ArrayType == "linear":
                obj.setEditorMode("NumberX", 2)
                obj.setEditorMode("NumberY", 2)
                obj.setEditorMode("NumberZ", 2)
                obj.setEditorMode("NumberPolar", 2)
                obj.setEditorMode("CenterAxis", 2)
                obj.setEditorMode("OrthoFace", 2)
                obj.setEditorMode("IntervalX", 0)
                obj.setEditorMode("IntervalY", 0)
                obj.setEditorMode("IntervalZ", 0)
                obj.setEditorMode("Number", 0)
                sh = self.linearArray(obj.Base.Shape, obj.IntervalX.Value, obj.IntervalY.Value, obj.IntervalZ.Value,
                                      obj.Number)

            elif obj.ArrayType == "ortho":
                # FreeCAD.Console.PrintMessage(obj.IntervalX.x)
                # 隐藏圆形阵列属性，显示矩形阵列属性
                obj.setEditorMode("NumberPolar", 2)
                obj.setEditorMode("Number", 2)
                obj.setEditorMode("CenterAxis", 2)
                obj.setEditorMode("OrthoFace", 0)
                if obj.OrthoFace == "XY":
                    obj.setEditorMode("NumberX", 0)
                    obj.setEditorMode("NumberY", 0)
                    obj.setEditorMode("NumberZ", 2)
                    obj.setEditorMode("IntervalX", 0)
                    obj.setEditorMode("IntervalY", 0)
                    obj.setEditorMode("IntervalZ", 2)
                    sh = self.rectArray(obj.Base.Shape, obj.IntervalX.Value, obj.IntervalY.Value, 0, obj.NumberX,
                                        obj.NumberY, 1)
                elif obj.OrthoFace == "XZ":
                    obj.setEditorMode("NumberX", 0)
                    obj.setEditorMode("NumberY", 2)
                    obj.setEditorMode("NumberZ", 0)
                    obj.setEditorMode("IntervalX", 0)
                    obj.setEditorMode("IntervalY", 2)
                    obj.setEditorMode("IntervalZ", 0)
                    sh = self.rectArray(obj.Base.Shape, obj.IntervalX.Value, 0, obj.IntervalZ.Value, obj.NumberX, 1,
                                        obj.NumberZ)
                elif obj.OrthoFace == "YZ":
                    obj.setEditorMode("NumberX", 2)
                    obj.setEditorMode("NumberY", 0)
                    obj.setEditorMode("NumberZ", 0)
                    obj.setEditorMode("IntervalX", 2)
                    obj.setEditorMode("IntervalY", 0)
                    obj.setEditorMode("IntervalZ", 0)
                    sh = self.rectArray(obj.Base.Shape, 0, obj.IntervalY.Value, obj.IntervalZ.Value, 1, obj.NumberY,
                                        obj.NumberZ)
            else:
                obj.setEditorMode("CenterAxis", 0)
                obj.setEditorMode("NumberPolar", 0)
                obj.setEditorMode("OrthoFace", 2)
                obj.setEditorMode("Number", 2)
                obj.setEditorMode("NumberX", 2)
                obj.setEditorMode("NumberY", 2)
                ...
                if obj.CenterAxis == "X":
                    ...
                    obj.Axis = Vector(0, 0, 1)
                sh = self.polarArray(obj.Base.Shape, obj.NumberPolar, obj.Axis)
            obj.Shape = sh
            if not DraftGeomUtils.isNull(pl):
                obj.Placement = pl

    def linearArray(self, shape, stepX, stepY, stepZ, num):
        import Part
        base = shape.copy()
        tool = []
        for count in range(num):
            translatevector = Vector(stepX, stepY, stepZ).multiply(count)
            if not count == 0:
                nshape = shape.copy()
                # FreeCAD.Console.PrintMess
                nshape.translate(translatevector)
                tool.append(nshape)
        # 这里将Part.makeCompound换成fuse，因为当模型重合时makecompound会有问题
        # return Part.makeCompound(base)
        # return base.fuse(tool)
        return base.multiFuse(tool).removeSplitter()

    def rectArray(self, shape, stepX, stepY, stepZ, numX, numY, numZ):
        import Part
        base = shape.copy()
        tool = []
        for xcount in range(numX):
            currentxvector = Vector(stepX, 0, 0).multiply(xcount)
            if not xcount == 0:
                nshape = shape.copy()
            ...
            for ycount in range(numY):
                ...
                currentyvector = currentyvector.add(Vector(0, stepY, 0).multiply(ycount))
                if not ycount == 0:
                    nshape = shape.copy()
                ...
                for zcount in range(numZ):
                    ...
                    currentzvector = currentzvector.add(Vector(0, 0, stepZ).multiply(zcount))
                    if not zcount == 0:
                        nshape = shape.copy()
    ...
    def polarArray(self, shape, num, axis):
        ...
        for i in range(num - 1):
            ...
            tool.append(nshape)
        # return base.fuse(tool)
        # return Part.makeCompound(tool)
        return base.fuse(tool).removeSplitter()


class _ArrayPolar(_DraftObject):
    "The Draft Array object"

    def __init__(self, obj):
        _DraftObject.__init__(self, obj, "Array")
        # print obj.supportedProperties()
        obj.addProperty("App::PropertyLink", "Base", "Object of an Array",
                        QT_TRANSLATE_NOOP("App::Property", "The base object that must be duplicated"))
        obj.addProperty("App::PropertyEnumeration", "ArrayType", "Object of an Array",
                        QT_TRANSLATE_NOOP("App::Property", "The type of array to create"))
        obj.addProperty("App::PropertyString", "CenterAxis", "Object of an Array",
                        QT_TRANSLATE_NOOP("App::Property", "The axis of array to rotate"))
        obj.addProperty("App::PropertyString", "OrthoFace", "Object of an Array",
                        QT_TRANSLATE_NOOP("App::Property", "The face of copies"))
        obj.addProperty("App::PropertyInteger", "NumberR", "Object of an Array",
                        QT_TRANSLATE_NOOP("App::Property", "Number of copies in R direction"))
        obj.addProperty("App::PropertyInteger", "NumberZ", "Object of an Array",
                        QT_TRANSLATE_NOOP("App::Property", "Number of copies in Z direction"))
        obj.addProperty("App::PropertyInteger", "Number", "Object of an Array",
                        QT_TRANSLATE_NOOP("App::Property", "Number of copies in linear"))
        obj.addProperty("App::PropertyInteger", "NumberPolar", "Object of an Array",
                        QT_TRANSLATE_NOOP("App::Property", "Number of copies"))
        obj.addProperty("App::PropertyDistance", "IntervalR", "Object of an Array",
                        QT_TRANSLATE_NOOP("App::Property", "Distance and orientation of intervals in R direction"))
        obj.addProperty("App::PropertyAngle", "IntervalTheta", "Object of an Array",
                        QT_TRANSLATE_NOOP("App::Property", "Distance and orientation of intervals in Theta direction"))
        obj.addProperty("App::PropertyDistance", "IntervalZ", "Object of an Array",
                        QT_TRANSLATE_NOOP("App::Property", "Distance and orientation of intervals in Z direction"))
        obj.addProperty("App::PropertyString", "Type", "", "Type of Object").Type = ObjectTools.ObjectType.Vol_Array
        obj.addProperty("App::PropertyString", "BaseType", "", "Type of Base Object").BaseType = "BaseType"
    ...
    def execute(self, obj):
        ...
        if obj.Base:
            ...
            if obj.ArrayType == "polar":
                sh = self.polarArray(obj.Base.Shape, obj.NumberPolar)
                obj.setEditorMode("NumberR", 2)
                obj.setEditorMode("NumberZ", 2)
                obj.setEditorMode("Number", 2)
                obj.setEditorMode("IntervalR", 2)
                obj.setEditorMode("IntervalTheta", 2)
                obj.setEditorMode("IntervalZ", 2)
                obj.setEditorMode("OrthoFace", 2)
                obj.setEditorMode("CenterAxis", 1)
                obj.setEditorMode("NumberPolar", 0)
                ...
                obj.setEditorMode("NumberPolar", 2)
                obj.setEditorMode("CenterAxis", 2)
                obj.setEditorMode("IntervalR", 0)
                obj.setEditorMode("IntervalZ", 0)
                if obj.ArrayType == "ortho":
                    obj.setEditorMode("IntervalTheta", 2)
                    obj.setEditorMode("Number", 2)
                    obj.setEditorMode("OrthoFace", 1)
                    obj.setEditorMode("NumberR", 0)
                    obj.setEditorMode("NumberZ", 0)
                    sh = self.rectArray(obj.Base, obj.IntervalR.Value, obj.IntervalZ.Value, obj.NumberR, obj.NumberZ)
                elif obj.ArrayType == "linear":
                    obj.setEditorMode("IntervalTheta", 0)
                    obj.setEditorMode("Number", 0)
                    obj.setEditorMode("NumberR", 2)
                    obj.setEditorMode("NumberZ", 2)
                    obj.setEditorMode("OrthoFace", 2)
                    sh = self.linearArray(obj.Base, obj.IntervalR.Value, obj.IntervalTheta.Value, obj.IntervalZ.Value,
                                          obj.Number)
            obj.Shape = sh
            if not DraftGeomUtils.isNull(pl):
                obj.Placement = pl
    ...
    #     if prop == "Label":
    #         label = obj.Label
    #         ObjectTools.updateWhenLableChanged(obj, self.labelBefore, label)

    def rectArray(self, base, stepR, stepZ, numR, numZ):
        import Part
        # base 所有属性及其值
        propertiesValues = {i: getattr(base, i) for i in base.PropertiesList}
        # print propertiesValues
        # 当前模型的所有坐标点
        points = []
        for val in propertiesValues:
            # val 为 key，可以通过 propertiesValues[val] 获取对应的值
            if "Point" in val:
                points.append(val)
        # list = [base.Shape.copy()]
        baseShape = base.Shape.copy()
        toolShape = []
        # r 轴进行阵列
        for rcount in range(numR):
            pass
            if not rcount == 0:
                objCopyX = FreeCAD.ActiveDocument.copyObject(base)
                objCopyX.removeProperty("Order")
                objCopyX.removeProperty("Type")
                properties = objCopyX.PropertiesList
                for point in points:
                    pointValue = propertiesValues[point]
                    r = pointValue.x
                    theta = pointValue.y
                    z = pointValue.z
                    r = r + stepR
                    propertiesValues[point] = Vector(r, theta, z)
                    for pro in properties:
                        if "Point" in pro and pro == point:
                            setattr(objCopyX, pro, Vector(r, theta, z))
                # 重新计算 objCopyX，更新顶点坐标
                objCopyX.recompute()
                toolShape.append(objCopyX.Shape.copy())
                FreeCADGui.activeDocument().getObject(objCopyX.Name).Visibility = False
                FreeCAD.activeDocument().removeObject(objCopyX.Name)
        # rsh = Part.makeCompound(list)
        rsh = baseShape.fuse(toolShape).removeSplitter()
        ...
        toolShapeZ = []
        for zcount in range(numZ):
            translateVec = Vector(0, 0, stepZ).multiply(zcount)
            if not zcount == 0:
                nrsh = rsh.copy()
                nrsh.translate(translateVec)
                toolShapeZ.append(nrsh)
        # return Part.makeCompound(sh)
        return baseShapeZ.fuse(toolShapeZ).removeSplitter()

    def linearArray(self, base, stepR, stepTheta, stepZ, num):
        import Part
        # base 所有属性及其值
        propertiesValues = {i: getattr(base, i) for i in base.PropertiesList}
        # print propertiesValues
        ...
        for count in range(num):
            if not count == 0:
                ...
                # 移除复制对象的 Order 属性和 Type 属性
                objCopy.removeProperty("Order")
                objCopy.removeProperty("Type")
                properties = objCopy.PropertiesList
                # print("points")
                for point in points:
                    pointValue = propertiesValues[point]
                    r = pointValue.x
                    ...
                    for pro in properties:
                        if "Point" in pro and pro == point:
                            objCopy.setExpression(pro + ".x", str(r))
                            objCopy.setExpression(pro + ".y", str(theta))
                            objCopy.setExpression(pro + ".z", str(z))
                            # setattr(objCopy, pro, Vector(r, theta, z))
                objCopy.recompute()
...
class _ArrayCylindrical(_DraftObject):
    "The Draft Array object"

    def __init__(self, obj):
        _DraftObject.__init__(self, obj, "Array")
        # print obj.supportedProperties()
        obj.addProperty("App::PropertyLink", "Base", "Attribute",
                        QT_TRANSLATE_NOOP("App::Property", "The base object that must be duplicated"))
        obj.addProperty("App::PropertyEnumeration", "ArrayType", "Attribute",
                        QT_TRANSLATE_NOOP("App::Property", "The type of array to create"))
        obj.addProperty("App::PropertyInteger", "NumberR", "Attribute",
                        QT_TRANSLATE_NOOP("App::Property", "Number of copies in R direction"))
        obj.addProperty("App::PropertyInteger", "NumberZ", "Attribute",
                        QT_TRANSLATE_NOOP("App::Property", "Number of copies in Z direction"))
        obj.addProperty("App::PropertyInteger", "Number", "Attribute",
                        QT_TRANSLATE_NOOP("App::Property", "Number of copies in linear"))
        obj.addProperty("App::PropertyInteger", "NumberPolar", "Attribute",
                        QT_TRANSLATE_NOOP("App::Property", "Number of copies"))
        obj.addProperty("App::PropertyLength", "IntervalZ", "Attribute",
                        QT_TRANSLATE_NOOP("App::Property", "Distance and orientation of intervals in Z direction"))
        obj.addProperty("App::PropertyLength", "IntervalR", "Attribute",
                        QT_TRANSLATE_NOOP("App::Property", "Distance and orientation of intervals in R direction"))
        obj.addProperty("App::PropertyAngle", "IntervalTheta", "Attribute",
                        QT_TRANSLATE_NOOP("App::Property", "Distance and orientation of intervals in Theta direction"))
        obj.addProperty("App::PropertyAngle", "Angle", "Attribute",
                        QT_TRANSLATE_NOOP("App::Property", "Angle to cover with copies"))
        obj.ArrayType = ['linear', 'ortho', 'polar']
        obj.NumberR = 1
        obj.NumberZ = 1
        obj.Number = 1
    ...
    def execute(self, obj):
        ...
        if obj.Base:
            pl = obj.Placement
            if obj.ArrayType == "polar":
                sh = self.polarArray(obj.Base.Shape, obj.Angle.Value, obj.NumberPolar)
                obj.setEditorMode("NumberR", 2)
                obj.setEditorMode("NumberZ", 2)
                obj.setEditorMode("Number", 2)
                obj.setEditorMode("IntervalR", 2)
    ...
    def rectArray(self, base, stepR, stepZ, numR, numZ):
        ...
        for rcount in range(numR):
            if not rcount == 0:
                ...
                for point in points:
                    ...
                    theta = pointValue.z
                    r = r + stepR
                    propertiesValues[point] = Vector(z, r, theta)
                    for pro in properties:
                        if "Point" in pro and pro == point:
                            objCopy.setExpression(pro + ".x", str(z))
                            objCopy.setExpression(pro + ".y", str(r))
                            objCopy.setExpression(pro + ".z", str(theta))
                            # setattr(objCopyX, pro, Vector(z, r, theta))
                # 重新计算 objCopyX，更新顶点坐标
                objCopyX.recompute()
                list.append(objCopyX.Shape.copy())
                FreeCADGui.activeDocument().getObject(objCopyX.Name).Visibility = False
                FreeCAD.activeDocument().removeObject(objCopyX.Name)
        rsh = Part.makeCompound(list)
        ...
        sh = [rsh.copy()]
        for zcount in range(numZ):
            translateVec = Vector(stepZ, 0, 0).multiply(zcount)
            if not zcount == 0:
                nrsh = rsh.copy()
                nrsh.translate(translateVec)
                sh.append(nrsh)
                ...
                for point in points:
                    ...
                    for pro in properties:
                        if "Point" in pro and pro == point:
                            ...
                            objCopy.setExpression(pro + ".y", str(r))
                            objCopy.setExpression(pro + ".z", str(theta))
                            # setattr(objCopy, pro, Vector(z, r, theta))
                objCopy.recompute()
                list.append(objCopy.Shape.copy())
                FreeCADGui.activeDocument().getObject(objCopy.Name).Visibility = False
                FreeCAD.activeDocument().removeObject(objCopy.Name)
        return Part.makeCompound(list)

    def polarArray(self, shape, angle, num):
        # print("angle ",angle," num ",num)
        center = Vector(0, 0, 0)
        axis = Vector(0, 0, 1)
        axisvector = None
...
class _PathArray(_DraftObject):
    ...
    def __init__(self, obj):
        ...
        obj.addProperty("App::PropertyLink", "PathObj", "Draft",
                        QT_TRANSLATE_NOOP("App::Property", "The path object along which to distribute objects"))
        obj.addProperty("App::PropertyLinkSubList", "PathSubs",
                        QT_TRANSLATE_NOOP("App::Property", "Selected subobjects (edges) of PathObj"))
        obj.addProperty("App::PropertyInteger", "Count", "Draft",
                        QT_TRANSLATE_NOOP("App::Property", "Number of copies"))
        obj.addProperty("App::PropertyVectorDistance", "Xlate", "Draft",
                        QT_TRANSLATE_NOOP("App::Property", "Optional translation vector"))
        obj.addProperty("App::PropertyBool", "Align", "Draft",
                        QT_TRANSLATE_NOOP("App::Property", "Orientation of Base along path"))
        obj.Count = 2
        obj.PathSubs = []
...
class _Clone(_DraftObject):
    ...
    def __init__(self, obj):
        ...
        obj.addProperty("App::PropertyBool", "Fuse", "Draft", QT_TRANSLATE_NOOP("App::Property",
                                                                                "If this clones several objects, this specifies if the result is a fusion or a compound"))
...
class _ShapeString(_DraftObject):
    ...
    def __init__(self, obj):
        ...
        obj.addProperty("App::PropertyLength", "Tracking", "Draft",
                        QT_TRANSLATE_NOOP("App::Property", "Inter-character spacing"))
...
class _Facebinder(_DraftObject):
    ...
    def __init__(self, obj):
        ...
        obj.addProperty("App::PropertyLinkSubList", "Faces", "Draft",
                        QT_TRANSLATE_NOOP("App::Property", "Linked faces"))
        obj.addProperty("App::PropertyBool", "RemoveSplitter", "Draft",
                        QT_TRANSLATE_NOOP("App::Property", "Specifies if splitter lines must be removed"))
        obj.addProperty("App::PropertyDistance", "Extrusion", "Draft",
                        QT_TRANSLATE_NOOP("App::Property", "An optional extrusion value to be applied to all faces"))
    ...
    def execute(self, obj):
        ...
        try:
            ...
            sayz("Draft: error building facebinder")
            return
        obj.Shape = sh
        obj.Placement = pl
...
class ViewProviderWorkingPlaneProxy:
    ...
    def __init__(self, vobj):
        ...
        vobj.addProperty("App::PropertyLength", "ArrowSize", "Arch",
    ...
    def attach(self, vobj):
        ...
        ls.coordIndex.setValues(0, 28,
                                [0, 1, -1, 2, 3, 4, 5, -1, 6, 7, -1, 8, 9, 10, 11, -1, 12, 13, -1, 14, 15, 16, 17, -1,
                                 18, 19, 20, 21])
...
class DraftLabel:
    ...
    def __init__(self, obj):
        ...
        obj.addProperty("App::PropertyPlacement", "Placement", "Base",
                        QT_TRANSLATE_NOOP("App::Property", "The placement of this object"))
        obj.addProperty("App::PropertyDistance", "StraightDistance", "Base",
                        QT_TRANSLATE_NOOP("App::Property", "The length of the straight segment"))
        obj.addProperty("App::PropertyVector", "TargetPoint", "Base",
                        QT_TRANSLATE_NOOP("App::Property", "The point indicated by this label"))
        obj.addProperty("App::PropertyVectorList", "Points", "Base",
                        QT_TRANSLATE_NOOP("App::Property", "The points defining the label polyline"))
        obj.addProperty("App::PropertyEnumeration", "StraightDirection", "Base",
                        QT_TRANSLATE_NOOP("App::Property", "The direction of the straight segment"))
        obj.addProperty("App::PropertyEnumeration", "LabelType", "Base",
                        QT_TRANSLATE_NOOP("App::Property", "The type of information shown by this label"))
        obj.addProperty("App::PropertyLinkSub", "Target", "Base",
                        QT_TRANSLATE_NOOP("App::Property", "The target object of this label"))
        obj.addProperty("App::PropertyStringList", "CustomText", "Base",
                        QT_TRANSLATE_NOOP("App::Property", "The text to display when type is set to custom"))
        obj.addProperty("App::PropertyStringList", "Text", "Base",
                        QT_TRANSLATE_NOOP("App::Property", "The text displayed by this label"))
        self.Type = "Label"
            ...
            if obj.LabelType == "Name":
                ...
                if obj.Target[0].isDerivedFrom("Part::Feature"):
                    ...
                    if "Edge" in obj.Target[1][0]:
                        obj.Text = [
                            FreeCAD.Units.Quantity(obj.Target[0].Shape.Edges[int(obj.Target[1][0][4:]) - 1].Length,
                ...
                if obj.Target[0].isDerivedFrom("Part::Feature"):
                    ...
                    if "Face" in obj.Target[1][0]:
                        obj.Text = [
                            FreeCAD.Units.Quantity(obj.Target[0].Shape.Faces[int(obj.Target[1][0][4:]) - 1].Area,
                                                   FreeCAD.Units.Area).UserString]
...
class ViewProviderDraftLabel:
    ...
    def __init__(self, vobj):
        vobj.addProperty("App::PropertyLength", "TextSize", "Base",
                         QT_TRANSLATE_NOOP("App::Property", "The size of the text"))
        vobj.addProperty("App::PropertyFont", "TextFont", "Base",
                         QT_TRANSLATE_NOOP("App::Property", "The font of the text"))
        vobj.addProperty("App::PropertyLength", "ArrowSize", "Base",
                         QT_TRANSLATE_NOOP("App::Property", "The size of the arrow"))
        vobj.addProperty("App::PropertyEnumeration", "TextAlignment", "Base",
                         QT_TRANSLATE_NOOP("App::Property", "The vertical alignment of the text"))
        vobj.addProperty("App::PropertyEnumeration", "ArrowType", "Base",
                         QT_TRANSLATE_NOOP("App::Property", "The type of arrow of this label"))
        vobj.addProperty("App::PropertyEnumeration", "Frame", "Base",
                         QT_TRANSLATE_NOOP("App::Property", "The type of frame around the text of this object"))
        ...
        vobj.addProperty("App::PropertyInteger", "MaxChars", "Base", QT_TRANSLATE_NOOP("App::Property",
