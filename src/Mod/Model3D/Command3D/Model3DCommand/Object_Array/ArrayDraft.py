...
'''
FreeCAD 0.17 Draft.py
位置：\FreeCAD 0.17\Mod\Draft\Draft.py
改动：1. _ViewProviderDraft.onChanged 添加了 if hasattr(self, "texture") 和 if hasattr(self, "texcoords")
     2.代码末尾定义了 protected_ViewProviderDraftArray() 函数，封装了受保护的 _ViewProviderDraft() 模块，以便外部调用
     3.定义了 makeArray3D() 函数，重写原 makeArray() 的功能
     4.定义了 makeArray3D_Polar() 函数，实现极坐标系下阵列的建模，其中重写_Array类，命名为_ArrayPolar
'''
...
class _Array(_DraftObject):
    ...
    def __init__(self, obj):
        ...
        obj.addProperty("App::PropertyVectorDistance", "IntervalX", "Draft",
                        QT_TRANSLATE_NOOP("App::Property", "Distance and orientation of intervals in X direction"))
        obj.addProperty("App::PropertyVectorDistance", "IntervalY", "Draft",
                        QT_TRANSLATE_NOOP("App::Property", "Distance and orientation of intervals in Y direction"))
        obj.addProperty("App::PropertyVectorDistance", "IntervalZ", "Draft",
                        QT_TRANSLATE_NOOP("App::Property", "Distance and orientation of intervals in Z direction"))
        obj.addProperty("App::PropertyVectorDistance", "IntervalAxis", "Draft",
                        QT_TRANSLATE_NOOP("App::Property", "Distance and orientation of intervals in Axis direction"))
        obj.addProperty("App::PropertyVectorDistance", "Center", "Draft",
                        QT_TRANSLATE_NOOP("App::Property", "Center point"))
        obj.addProperty("App::PropertyAngle", "Angle", "Draft",
                        QT_TRANSLATE_NOOP("App::Property", "Angle to cover with copies"))
        obj.addProperty("App::PropertyBool", "Fuse", "Draft",
                        QT_TRANSLATE_NOOP("App::Property", "Specifies if copies must be fused (slower)"))
...
class _ArrayPolar(_DraftObject):
    """
    重写_Array类，_ArrayPolar使其能在极坐标下成功建模
    """
    "The Draft Array object"
    def __init__(self, obj):
        _DraftObject.__init__(self, obj, "Array")
    ...
    def execute(self, obj):
        ...
        if obj.Base:
            ...
            if obj.ArrayType == "linear":
                sh = self.linearArray(obj.Base,
                                      obj.IntervalR.Value, obj.IntervalTheta.Value, obj.IntervalZ.Value,
                                      obj.Number)
            elif obj.ArrayType == "ortho":
                sh = self.rectArray(obj.Base, obj.IntervalR.Value, obj.IntervalZ.Value, obj.NumberR, obj.NumberZ)
            else:
                sh = self.polarArray(obj.Base, obj.NumberPolar)
            obj.Shape = sh
            if not DraftGeomUtils.isNull(pl):
                obj.Placement = pl
                    ...
                    for pk in pointCoordinateKeys:
                        ...
                        if m:
                            if m.group(3) == 'X':
                                ...
                                pointValue = (pointValue + stepTheta) % 360.0
                            else:
                                pointValue += stepZ
                        else:
...
def makeArray3D(baseObject, arg1, arg2, arg3=None, arg4=None, arg5=None):
    """
    重写直角坐标系下的makeArray，返回一个物体obj
    """
    obj = FreeCAD.ActiveDocument.addObject("Part::FeaturePython", "tempArray")
    _Array(obj)
    obj.Base = baseObject

    if arg3 is not None and arg4 is not None and arg5 is None:
        # linear
        # 建模出来的模型只复制其Shape后将其删除，作过渡使用，其中"linear"属于"ortho"
        obj.ArrayType = "ortho"
        obj.IntervalX = Vector(arg1, arg2, arg3)
        obj.IntervalY = Vector(0, 0, 0)
        obj.NumberX = arg4
        obj.NumberY = 1

    elif arg3 is not None and arg4 is not None and arg5 is not None:
        # ortho
        vec1 = vec2 = Vector(0, 0, 0)
        if arg5 == "XY":
            vec1 = Vector(arg1, 0, 0)
            vec2 = Vector(0, arg2, 0)
        elif arg5 == "XZ":
            vec1 = Vector(arg1, 0, 0)
            vec2 = Vector(0, 0, arg2)
        elif arg5 == "YZ":
            vec1 = Vector(0, arg1, 0)
            vec2 = Vector(0, 0, arg2)
        else:
            Tools3D.sayz("Set Ortho face wrong!")
        obj.ArrayType = "ortho"
        obj.IntervalX = vec1
        obj.IntervalY = vec2
        obj.NumberX = arg3
    ...
    elif arg3 is None and arg4 is None and arg5 is None:
        if arg2 == "X":
            ...
            Tools3D.sayz("Set center axis wrong!")
        obj.ArrayType = "polar"
        obj.NumberPolar = arg1
    else:
        Tools3D.sayz("Parameters wrong!")
        return
...
def makeArray3D_Polar(baseObject, arg1, arg2, arg3=None, arg4=None, arg5=None):
    """
        重写极坐标系下的makeArray，返回一个物体obj
    """
    obj = FreeCAD.ActiveDocument.addObject("Part::FeaturePython", "tempArray")
    # 创建baseObj的复制，建立临时阵列体时不影响原baseObj
    _ArrayPolar(obj)
    obj.Base = baseObject

    if arg3 is not None and arg4 is not None and arg5 is None:
        # linear
        obj.ArrayType = "linear"
        obj.IntervalR = arg1
        obj.IntervalTheta = arg2
        obj.IntervalZ = arg3
        obj.Number = arg4
    elif arg3 is not None and arg4 is not None and arg5 is not None:
        # ortho
...
def protected_ViewProviderDraftArray(obj):
    """
    由于_ViewProviderDraftArray()属于受保护模块，故封装以便外部调用
    """
    _ViewProviderDraftArray(obj)


def ViewProvider_Array(obj):
    if gui:
        _ViewProviderDraftArray(obj.ViewObject)
        formatObject(obj, obj.Base)
