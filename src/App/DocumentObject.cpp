...
/*fubiao*/
//std::map<std::string, std::string> getPropMapToParam(){
//	//先获得ExpressionEngine
//	PropertyExpressionEngine expEng = this->ExpressionEngine;
//}
/*fubiao*/
/**
...
void DocumentObject::onChanged(const Property* prop)
{
	...
	if (!(prop->getType() & Prop_Output))
	{
		...
		//	if (strcmp(this->getNameInDocument(), "Param") == 0){
		//		std::vector<std::string> props = static_cast<PropertyStringList*>(_pDoc->getPropertyByName("PropertiesChanged"))->getValues();
		//		props.push_back(prop->getName());
		//		static_cast<PropertyStringList*>(_pDoc->getPropertyByName("PropertiesChanged"))->setValues(props);

		//		std::vector<std::string> propsTest = static_cast<PropertyStringList*>(_pDoc->getPropertyByName("PropertiesChanged"))->getValues();
		//	}
		//}

		//addTouchedProps(prop->getName());
		//touchedProps.push_back(prop);
	}
