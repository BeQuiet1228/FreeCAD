...
void DynamicProperty::getPropertyList(std::vector<Property*> &List) const
{
	...
	//for (std::vector<std::pair<std::string, PropData>>::const_iterator it = props.begin(); it != props.end(); ++it)
...
Property *DynamicProperty::getPropertyByName(const char* name) const
{
	...
	//std::vector<std::pair<std::string, PropData>>::const_iterator it = props.end();
	//_ASSERTE(_CrtCheckMemory());
	//for (std::vector<std::pair<std::string, PropData>>::const_iterator iter = props.begin(); iter != props.end(); iter++)
	//	if (iter->first == name){
	//		it = iter;
	//	}
	//_ASSERTE(_CrtCheckMemory());
                                              ...
                                              const char* doc, short attr, bool ro, bool hidden)
{
	...
	//props.push_back(std::pair<std::string, PropData>(ObjectName, data));
	//_ASSERTE(_CrtCheckMemory());
...
void DynamicProperty::Save (Base::Writer &writer) const 
{
    ...
    for (it = Map.begin(); it != Map.end(); ++it)
    {
		...
		//std::vector<std::pair<std::string, PropData>>::const_iterator pt = props.end();
		//for (std::vector<std::pair<std::string, PropData>>::const_iterator iter = props.begin(); iter != props.end(); iter++)
