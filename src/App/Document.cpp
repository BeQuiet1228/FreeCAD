	...
	:classID(0)
{
	...
	///*fubiao*/
	//std::list<std::string> WorkRangeList = App::GetApplication().GetParameterGroupByPath
	//	("User parameter:BaseApp/Preferences/Document")->GetASCII("prefWorkRangeList", "");
	///*end*/
	//坐标系数据的存储--------------------
	int coordinateSystemId = App::GetApplication().GetParameterGroupByPath
		("User parameter:BaseApp/Preferences/Document")->GetInt("prefCoordinateSystemId", 0);
	...
	//int whichOrder = -1;
	ADD_PROPERTY_TYPE(CoordinateSystem, (coordinateSystem.c_str()), 0, Prop_None, "Set Coordinate System");
	ADD_PROPERTY_TYPE(flagNeedUpdateBoolean, (-1), 0, Prop_None, " Is the document need recompute boolean obj?");

	ADD_PROPERTY_TYPE(PropertiesChanged, (""), 0, Prop_None, " Is the document need recompute boolean obj?");

	ADD_PROPERTY_TYPE(Begin, ("{}"), 0, Prop_None, "custom define");

	//坐标系数据的存储----End--------------------
	///*fubiao*/
	//ADD_PROPERTY_TYPE(WorkRange, (WorkRangeList), 0, Prop_None, "Set Work Range");
	///*end*/
    ...
    for (auto objIt = topoSortedObjects.rbegin(); objIt != topoSortedObjects.rend(); ++objIt){
        ...
        if ((*objIt)->isTouched() || (*objIt)->mustExecute() == 1){
			...
			//signalRecomputedObject(*(*objIt));
				...
				(*objIt)->getPropertyList(properties);
				bool isParamObj = false;
				for (std::vector<Property*>::iterator it = properties.begin();
					it != properties.end(); it++){
					if (strcmp((*it)->getName(), "DynamicData") == 0){
						isParamObj = true;
						break;
					}
				}
				//判断是不是参数体
				if (isParamObj){
					Document* pcDoc = (*objIt)->getDocument();
					//获取参数体改变的属性
					PropertyString* changedProps = static_cast<PropertyString*>(pcDoc->getPropertyByName("PropertiesChanged"));
					std::string propNamesValue = changedProps->getValue();
					std::vector<std::string> propNames;
					///按分号拆分名称
					boost::regex re(";");
					boost::sregex_token_iterator
						p(propNamesValue.begin(), propNamesValue.end(), re, -1);
					boost::sregex_token_iterator end;

					while (p != end){
						//空的值就不放进去了
						if ((*p)!="")
							propNames.push_back(*p);
						p++;
					}
					//propNames去重
					std::set<string> tempPropNames(propNames.begin(), propNames.end());
					propNames.assign(tempPropNames.begin(), tempPropNames.end());
					///
					std::vector<DocumentObject*> relateObjs = (*objIt)->getInList();
					//relateObjs去重
					std::set<DocumentObject*> tempRelateObjs(relateObjs.begin(), relateObjs.end());
					relateObjs.assign(tempRelateObjs.begin(), tempRelateObjs.end());
					//再遍历这些相关的模型，若与之相关的属性会影响建模，则令该模型需要被刷新
					for (std::vector<App::DocumentObject*>::iterator iter = relateObjs.begin();
						iter != relateObjs.end(); ++iter){
						//
						bool isNeedTouch = false;
						//获得表达式器中的属性（引用paramObj中的属性必在其中）
						PropertyExpressionEngine* expEng = static_cast<PropertyExpressionEngine*>((*iter)->getPropertyByName("ExpressionEngine"));
						//boost::unordered_map<int, int> x;
						boost::unordered_map<const App::ObjectIdentifier, const App::PropertyExpressionEngine::ExpressionInfo> expEngValues = expEng->getExpressions();
						//遍历epressionEnge
						boost::unordered_map<const App::ObjectIdentifier, const App::PropertyExpressionEngine::ExpressionInfo>::iterator expEngIter;
						for (expEngIter = expEngValues.begin(); expEngIter != expEngValues.end(); ++expEngIter){
							App::Property* prop = expEngIter->first.getProperty();
							const char* test = prop->getGroup();
							const char*test2 = prop->getDocumentation();
							//判断这个模型的这个属性是否影响建模

							if (!(strcmp(prop->getGroup(), "Base") == 0 || boost::starts_with(prop->getGroup(),"Object"))){//属性分组是不是以Object开头的
								continue;
							}
							//后面就是会影响建模的属性，判断表达式中的参数对象的属性值是否在变化的参数列表中
							std::string expStr = expEngIter->second.expression->toString();
							//分割
							boost::regex re("\\.|\\+|-|\\*|\\/|\\(|\\)|\\s+");
							boost::sregex_token_iterator 
								p(expStr.begin(), expStr.end(), re, -1);
							boost::sregex_token_iterator end;
							std::vector<std::string> expressionSingles;
							while (p != end){
								if (*p != "")
									expressionSingles.push_back(*p);
								p++;
							}
							//判断expressionSigles 和 propsNames有没有交集
							std::string paramObjName = (*objIt)->getNameInDocument();
							for (std::vector<std::string>::iterator propNameIt = propNames.begin();
								propNameIt != propNames.end(); ++propNameIt)
							{
								for (std::vector<std::string>::iterator expIt = expressionSingles.begin();
									expIt != expressionSingles.end();++expIt)
									if (strcmp((*propNameIt).c_str(), (*expIt).c_str()) == 0){
										isNeedTouch = true;
										break;
									}
								if (isNeedTouch)
							...
							//std::string s4 = expEngIter->first.getValue();
					...
					static_cast<PropertyString*>(pcDoc->getPropertyByName("PropertiesChanged"))->setValue("");
					///
					////遍历这些属性名，找到会影响别的模型的属性
					//for (std::vector<std::string>::iterator it = propNames.begin();
					//	it != propNames.end(); ++it)
					//{
					//	Property* prop = (*objIt)->getPropertyByName((*it).c_str());
					//	if (prop == NULL){
					//		continue;
					//	}
					//	std::vector<DocumentObject*> relateObjs = (*objIt)->getInList();
					...
					//		boost::unordered_map<const App::ObjectIdentifier, const App::PropertyExpressionEngine::ExpressionInfo>::iterator expEngIter;
					//		for (expEngIter = expEngValues.begin(); expEngIter != expEngValues.end(); ++expEngIter){
					//			std::string s1=expEngIter->first.getPropertyName();
					//			std::string s2 = expEngIter->first.toString();
					//			std::string s3 = expEngIter->first.getSubPathStr();
					//			//std::string s4 = expEngIter->first.getValue();
					//			std::string s4 = expEngIter->second.comment;
					//		}
					//	}
					//}
				}
				...
				//for (auto inObjIt : (*objIt)->getInList())
				//{
				//	//std::vector<App::Property*> propsList;
				//	//inObjIt->getPropertyList(propsList);
				//	//std::vector<App::Property*> touchedProps = (*objIt)->getTouchedProps();
				//	////判断两个vector是否有相等 相等则需要刷新
				//	//bool isneedTouch = false;
				//	//for (std::vector<App::Property*>::iterator it = propsList.begin();
				//	//	it != propsList.end(); ++it){
				//	//	for (std::vector<App::Property*>::iterator iter = touchedProps.begin();
				//	//		iter != touchedProps.end(); ++iter){
				//	//		if ((*it) == (*iter)){
				//	//			isneedTouch = true;
				//	//			break;
				//	//		}
