...
public:
	...
	///获得与参数对象相关的属性
	//void addTouchedProps(std::string propName){
	//	touchedProps.push_back(propName);
	//}
	//std::vector<std::string> getTouchedProps(){
	//	return touchedProps;
	//}
	//void clearTouchedProps(){
	//	for (std::vector<std::string>::iterator it =
	//		touchedProps.begin(); it != touchedProps.end(); ++it) {
	//		//delete *it;
	//	}
	//	touchedProps.clear();
	//}
	///
	//std::map<string, string> getPropMapToParam();
...
private:
	...
	///使得该模型touch的属性列表，设置touch时添加，清除touch时，移除
	//std::vector<std::string> touchedProps;
	///与属性对象相关且影响建模的属性名与属性对象的属性名Map
	//std::map<string, string> _propsMapToParam;
};
