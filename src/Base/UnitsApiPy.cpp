...
PyMethodDef UnitsApi::Methods[] = {
	...
	/*fubiao*/
	{
		"setDefaultUnits", (PyCFunction)UnitsApi::sSetDefaultUnits, 1,
		"setDefaultUnits(Length,Angle,TimeSpan,ElectricCurrent,ElectricPotential,Frequency) -> tuple\n\n"
		"Set Default Units"
	},
	{
		"getDefaultUnits", (PyCFunction)UnitsApi::sGetDefaultUnits, 1,
		"getDefaultUnits() return Length,Angle,TimeSpan,ElectricCurrent,ElectricPotential,Frequency -> tuple\n\n"
		"Set Default Units"
	},
...
/*fubiao*/
PyObject* UnitsApi::sSetDefaultUnits(PyObject * /*self*/, PyObject *args, PyObject * /*kwd*/)
{
	int sLength = 0;
	int sAngle = 0;
	int sTimeSpan = 0;
	int sElectricCurrent = 0;
	int sElectricPotential = 0;
	int sFrequency = 0;
	if (!PyArg_ParseTuple(args, "iiiiii", &sLength, &sAngle, &sTimeSpan, &sElectricCurrent, &sElectricPotential, &sFrequency))
		return NULL;


	if (-1<sLength<4){
		UnitsApi::defaultLengthUnit = sLength;
	}
	if (-1<sAngle<3){
		UnitsApi::defaultAngleUnit = sAngle;
	}
	if (-1<sTimeSpan<5){
		UnitsApi::defaultTimeSpanUnit = sTimeSpan;
	}
	if (-1<sElectricCurrent<4){
		UnitsApi::defaultElectricCurrentUnit = sElectricCurrent;
	}
	if (-1<sElectricPotential<4){
		UnitsApi::defaultElectricPotentialUnit = sElectricPotential;
	}
	if (-1<sFrequency<6){
		UnitsApi::defaultFrequencyUnit =sFrequency;
	}
	Py_Return;
}
PyObject* UnitsApi::sGetDefaultUnits(PyObject * /*self*/, PyObject *args, PyObject * /*kwd*/)
{
	if (!PyArg_ParseTuple(args, ""))
		return NULL;
	int lengthUnit = UnitsApi::defaultLengthUnit;
	int angleUnit = UnitsApi::defaultAngleUnit;
	int timeSpanUnit = UnitsApi::defaultTimeSpanUnit;
	int electricCurrentUnit = UnitsApi::defaultElectricCurrentUnit;
	int electricPotential = UnitsApi::defaultElectricPotentialUnit;
	int frequencyUnit= UnitsApi::defaultFrequencyUnit;
	return Py_BuildValue("[i,i,i,i,i,i]", lengthUnit, angleUnit, timeSpanUnit, electricCurrentUnit, electricPotential, frequencyUnit);
}
