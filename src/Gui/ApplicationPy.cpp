...
PyMethodDef Application::Methods[] = {
  ...
  {"getMainWindow",           (PyCFunction) Application::sGetMainWindow,    1,
   ...
   { "closeAllWindows", (PyCFunction)Application::sCloseAllWindows, 1,
  ...
  {"open",                    (PyCFunction) Application::sOpen,             1,
   ...
   { "chipicOpen", (PyCFunction)Application::sChipicOpen, 1,
   "Open a macro, Inventor or VRML file" },
...
PyObject* Application::sDisplayText(PyObject* self, PyObject* args, PyObject* kwd)
{
    ...
    //±£ÁôÖ®Ç°µÄ´°¿Ú×´Ì¬
    auto mdiView = mw->activeWindow();
    mw->addWindow(textEdit);
    mw->setActiveWindow(mdiView);
    textEdit->setText(QString::fromStdString(text));
...
PyObject* Application::sChipicOpen(PyObject * /*self*/, PyObject *args, PyObject * /*kwd*/)
{
	char* Name;
	...
	if (!fi.isFile() || !fi.exists()) {
		PyErr_Format(PyExc_IOError, "File %s doesn't exist.", EncodedName);
		return 0;
	}
	...
	else if (strcmp(suffix.c_str(), ".h5") == 0)
	{
...
PyObject* Application::sCloseAllWindows(PyObject * /*self*/, PyObject *args, PyObject * /*kwd*/)
{
	if (!PyArg_ParseTuple(args, ""))
		return NULL;
	Gui::getMainWindow()->closeAllWindows();
