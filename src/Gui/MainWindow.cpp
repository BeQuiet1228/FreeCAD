...
void MainWindow::onDockWindowMenuAboutToShow()
{
    QMenu* menu = static_cast<QMenu*>(sender());
    menu->clear();
    QStringList standardDockWidgets;
    standardDockWidgets
        << QString::fromLatin1("Toolbox")
        << QString::fromLatin1("Tree view")
        << QString::fromLatin1("Property view")
        << QString::fromLatin1("Selection view")
        << QString::fromLatin1("Combo View")
        << QString::fromLatin1("Report view")
        << QString::fromLatin1("Python console")
        << QString::fromLatin1("DAG View");
    ...
    for (QList<QDockWidget*>::Iterator it = dock.begin(); it != dock.end(); ++it) {
        ...
        //只有FreeCAD内置的面板会显示在视图-面板菜单里面…… @pingyue
        if (standardDockWidgets.contains((*it)->objectName()))
            menu->addAction(action);
    }
}
...
void MainWindow::setIconSize(const QSize &iconSize){
    ...
    auto toolbars = getInstance()->findChildren<QToolBar*>();
    for(auto t : toolbars){
        auto widgetActions = t->actions();
        for (auto widget : widgetActions){
            if (auto wa = qobject_cast<QWidgetAction*>(widget)){
                QLayout* layout = wa->defaultWidget()->layout();
                if (!layout)
                    continue;
                int count_item = layout->count();
                for (int i = 0; i < count_item; ++i){
                    auto item = layout->itemAt(i);
                    QWidget* w = item->widget();
                    if (auto b = qobject_cast<QToolButton*>(w)){
                        b->setFixedSize(QSize(height + 13, height + 13));
                        b->setIconSize(QSize(height, height));
                    }
                }
            }
        }
        //设置迭代次数标签的高度，让两个进度条始终对齐……
        if (auto label = t->findChild<QLabel*>(QString::fromUtf8("label_iterations")))
            label->setFixedHeight(height + 13);
    }
}
