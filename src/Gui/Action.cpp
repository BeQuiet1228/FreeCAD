...
/**
 * added by pingyue
 * Insert this action to widget \a w.
 */
void Action::insertTo(QWidget *w, QAction *before)
{
    w->insertAction(before, _action);
}

/**
 * added by pingyue
 * Set this action as default action of QToolButton \a btn.
 */
void Action::setDefaultAction(QToolButton *btn)
{
	btn->setDefaultAction(_action);
}
...
WorkbenchComboBox::WorkbenchComboBox(WorkbenchGroup* wb, QWidget* parent) : StyledComboBox(parent), group(wb)
{
...
void WorkbenchGroup::addTo(QWidget *w)
{
    ...
    if (w->inherits("QToolBar")) {
        ...
        StyledComboBox* box = new WorkbenchComboBox(this, w);
...
void WorkbenchGroup::refreshWorkbenchList()
{
    ...
    for (QStringList::Iterator it = enabled_wbs_list.begin(); it != enabled_wbs_list.end(); ++it) {
        ...
        if (index >= 0) {
			if (workbenchList.contains(Application::Instance->workbenchMenuText(*it))){
