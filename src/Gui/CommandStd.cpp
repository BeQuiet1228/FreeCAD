...
void StdCmdWorkbench::activated(int i)
{
    try {
		...
		//simulation,VisualWorkbench,M3DFileEditorWorkbench工作台中Property view面板设置为不可见
		if (switch_to == "PhysicsWorkbench" || switch_to == "VisualWorkbench" || switch_to == "M3DFileEditorWorkbench"){
			getMainWindow()->findChild<QDockWidget*>(QLatin1String("Property view"))->setVisible(false);
		}else{
			getMainWindow()->findChild<QDockWidget*>(QLatin1String("Property view"))->setVisible(true);
			}	
  ...
  :Command("Std_About")
{
    ...
    sMenuText     = QT_TR_NOOP("&About CHIPIC");
...
// Std_AboutQt
//===========================================================================
DEF_STD_CMD(StdCmdExample);

StdCmdExample::StdCmdExample()
	:Command("Std_Example")
{
	sGroup = QT_TR_NOOP("Help");
	sMenuText = QT_TR_NOOP("Example");
...
void StdCmdExample::activated(int iMsg)
{
	Q_UNUSED(iMsg);
	...
	QString project = QFileDialog::getOpenFileName(Gui::getMainWindow(),
		QString::fromUtf8(QT_TR_NOOP("Example")), QString::fromStdString(exampleStr),
...
void StdCmdOnlineHelp::activated(int iMsg)
{
    ...
    //帮助文件是/build/doc/help.chm
    std::string helpFilePath = "file:///" + App::Application::getHelpDir() + "help.chm";
...
void CreateStdCommands(void)
{
	...
	rcCmdMgr.addCommand(new StdCmdExample());
