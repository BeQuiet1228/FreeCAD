...
void TreeWidget::doubleClicked(){
	Gui::Command::runCommand(Gui::Command::Doc,
		"FreeCADGui.runCommand(\"ReShowDialog\")");
}
...
void TreeWidget::contextMenuEvent (QContextMenuEvent * e)
{
    ...
    if (this->contextItem && this->contextItem->type() == DocumentType) {
        ...
        // 把复制粘贴剪切命令插入到删除命令之前
        Gui::CommandManager &cm = Gui::Application::Instance->commandManager();
        Gui::Command *copyCommand = cm.getCommandByName("CopyCommand");
        Gui::Command *pasteCommand = cm.getCommandByName("PasteCommand");
        Gui::Command *cutCommand = cm.getCommandByName("CutCommand");
        
        QList<QAction*> acts = contextMenu.actions();
        QAction *deleteAction = nullptr;
        for (auto act : acts){
            if (act->objectName() == QLatin1String("Std_Delete"))
                deleteAction = act;
        }
        if (copyCommand && pasteCommand && cutCommand && deleteAction){
            copyCommand->insertTo(&contextMenu, deleteAction);
            pasteCommand->insertTo(&contextMenu, deleteAction);
            cutCommand->insertTo(&contextMenu, deleteAction);
            contextMenu.insertSeparator(deleteAction);
        }
	...
	std::vector<SelectionSingleton::SelObj> sel = Gui::Selection().getCompleteSelection();

	// 修改重命名的有效性
	this->relabelObjectAction->setEnabled(true);
	if (sel.size() == 1)
	{
		if (strcmp(sel[0].TypeName, "Part::FeaturePython") != 0 || sel[0].pObject->Label.getStrValue() == "ResultShape")
			this->relabelObjectAction->setEnabled(false);
	}
	else{
		this->relabelObjectAction->setEnabled(false);
	}
  ...
  : DockWindow(pcDocument,parent)
{
    ...
    this->treeWidget = new TreeWidgetm3d(this);
