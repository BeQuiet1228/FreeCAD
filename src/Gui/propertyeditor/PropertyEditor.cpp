...
void PropertyEditor::closeEditor (QWidget * editor, QAbstractItemDelegate::EndEditHint hint)
    ...
    if (autoupdate) {
		...
		if (doc && doc->isTouched()){
            doc->recompute();
			//Gui::Command::doCommand(Gui::Command::Doc, "FreeCAD.Console.PrintError(\"Start\\n\")");
			//else if(_numOfUpdate!=0){
			//	_numOfUpdate = 0;
			//Gui::Command::doCommand(Gui::Command::Doc, "from Modeling.Modeling3D.Modeling3DCommand.Vol_Boolean import BooleanCommand");
			Gui::Command::doCommand(Gui::Command::Doc, "DocumentTools.updateBoolean()");
			//}
			//Gui::Command::doCommand(Gui::Command::Doc, "FreeCAD.Console.PrintError(\"End\\n\")");
		}
    }
