...
void Command::insertTo(QWidget *pcWidget, QAction *before)
{
    if (!_pcAction)
        _pcAction = createAction();

    _pcAction->insertTo(pcWidget, before);
}

// 将action以toolbutton的形式添加到gridlayout
QToolButton* Command::addToGridLayout(QGridLayout *pcGridLayout, int row, int col, int rowStretch, int colStretch)
{
    if (!_pcAction)
        _pcAction = createAction();

    QToolButton* toolButton = new QToolButton(pcGridLayout->parentWidget());
    ParameterGrp::handle hGrp = WindowParameter::getDefaultParameter()->GetGroup("General");
    int size = hGrp->GetInt("ToolbarIconSize", 24);
    int iconSize = size;
    toolButton->setFixedSize(QSize(iconSize + (colStretch - 1) * 12 + 13, iconSize + (rowStretch - 1) * 12 + 13));
    toolButton->setAutoRaise(true);
    toolButton->setIconSize(QSize(iconSize + (colStretch - 1) * 12, iconSize + (rowStretch - 1) * 12));
    _pcAction->setDefaultAction(toolButton);
    pcGridLayout->addWidget(toolButton, row, col, rowStretch, colStretch);
    return toolButton;
}
...
QToolButton* CommandManager::addToGridLayout(const char* Name, QGridLayout *pcGridLayout, int row, int col, int rowStretch/* = 1*/, int colStretch/* = 1*/)
{
    if (_sCommands.find(Name) == _sCommands.end()) {
        // Print in release mode only a log message instead of an error message to avoid to annoy the user
#ifdef FC_DEBUG
        ...
        Command* pCom = _sCommands[Name];
        return pCom->addToGridLayout(pcGridLayout, row, col, rowStretch, colStretch);
    }
}
...
Gui::Action* Gui::CommandManager::creatAction(Command *cmd)
{
	...
	if (action)
		return action;
	action = cmd->createAction();
	cmd->_pcAction = action;
