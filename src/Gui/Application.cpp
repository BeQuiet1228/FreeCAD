...
Application::Application(bool GUIenabled)
{
    ...
    if (GUIenabled) {
        ...
        //修改默认语言为中文 @pingyue
        string language=hPGrp->GetASCII("Language");
        if (language.empty()){
            language = "Chinese Simplified";
            hPGrp->SetASCII("Language", language.c_str());
        }
		...
		/*默认单位*/
		Base::UnitsApi::setDefaultUnits(hUnits->GetInt("DefaultLengthUnit", Base::UnitsApi::getDefaultLength()),
			hUnits->GetInt("DefaultAngleUnit", Base::UnitsApi::getDefaultAngle()),
			hUnits->GetInt("DefaultTimeUnit", Base::UnitsApi::getDefaultTimeSpan()),
			hUnits->GetInt("DefaultEleCurUnit", Base::UnitsApi::getDefaultElectricCur()),
			hUnits->GetInt("DefaultElePotUnit", Base::UnitsApi::getDefaultElectircPot()),
			hUnits->GetInt("DefaultFrequencyUnit", Base::UnitsApi::getDefaultFrequency()));
...
void Application::open(const char* FileName, const char* Module)
{
    ...
    if (Module != 0) {
        ...
        try {
			...
			//设置初始化工作路径
			Command::doCommand(Command::App, "%s.clientSetWorkpath(\"%s\")", Module, filename.c_str());
...
void Application::setActiveDocument(Gui::Document* pcDocument)
{
    ...
    if (pcDocument){
		/*if (isDoubleClickedFile)
		{
			Command::doCommand(Command::App, "import Modeling\nModeling.Common.Tools.DocumentTools.initWhenOpenFCStdFile()\n");
			isDoubleClickedFile = false;

		}*/
		//std::cout << TraceStack() << std::endl;
...
//void Application::runApplication(std::string workpath)
...
//	mw.inittreeContor();
    ...
    int size = hGrp->GetInt("ToolbarIconSize", 24);
    ...
    //修改默认StyleSheet为Light-blue @pingyue
  /*  if (style.empty()){
	...
	////设置工作路径
	//Application::isDoubleClickedFile = false;
	//if (workpath.length() > 0)
	//{
	//	Command::doCommand(Command::App, "%s.clientSetWorkpath(\"%s\")", "FreeCAD", workpath.c_str());
	//	isDoubleClickedFile = true;
	//	//Command::doCommand(Command::App, "import Modeling\nModeling.Common.Tools.DocumentTools.initWhenOpenFCStdFile()\n");
	//}
    ...
    try {
		...
		std::string asciis = UTF_82ASCII(ss);
		/**/
		const char* fileName = asciis.c_str();
		/*std::string gbks = Utf8ToGbk(fileName);
		std::string uns = UTF82Char(fileName);*/
		///
